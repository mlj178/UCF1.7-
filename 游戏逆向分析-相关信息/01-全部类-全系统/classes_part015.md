...
- `string _typeName`（字符串 _类型名称）(偏移: 0x2C)
...
- `Quaternion defaultLocalRotation`（四元数 默认本地旋转）(偏移: 0x18)
- `bool initiated`（布尔值 已启动）(偏移: 0x29)
...
- `float min`（浮点数 最小值）(偏移: 0x30)
- `float max`（浮点数 最大值）(偏移: 0x34)
...
- `IntPtr value`（整数指针 value）(偏移: 0x0)
...
## SafeHandleZeroOrMinusOneIsInvalid（安全句柄零或负一是否无效）
...
- `ProfilingSampler m_ProfilingSampler`（性能分析采样器 m_性能分析采样器）(偏移: 0x60)
...
## ScriptableRendererFeature（可脚本化渲染器特性）
...
- `DrivenRectTransformTracker m_Tracker`（驱动矩形变换跟踪器 m_跟踪器）(偏移: 0xD4)
