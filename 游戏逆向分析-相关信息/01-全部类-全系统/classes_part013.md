...
- `IComparer comparer`（I比较器 comparer）(偏移: 0x1C)
...
- `int _size`（整数 _大小）(偏移: 0x14)
- `int _growFactor`（int _grow系数）(偏移: 0x18)
- `int _version`（整数 _版本）(偏移: 0x1C)
- `object _syncRoot`（对象 _同步根）(偏移: 0x20)
...
- `string m_Name`（字符串 m_名称）(偏移: 0x34)
...
- `Vector3 lastPosition`（三维向量 最后位置）(偏移: 0x40)
