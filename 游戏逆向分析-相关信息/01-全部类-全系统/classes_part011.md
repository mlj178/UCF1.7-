...
- `string _typeName`（字符串 _类型名称）(偏移: 0xC)
- `string _methodName`（string _method名称）(偏移: 0x10)
- `object[] _args`（对象[] _参数）(偏移: 0x14)
...
- `string _uri`（字符串 _uri）(偏移: 0xC)
- `string _typeName`（字符串 _类型名称）(偏移: 0x10)
...
- `Vector3 euler`（三维向量 欧拉角）(偏移: 0x14)
...
- `GraphNode node`（图节点 node）(偏移: 0x0)
- `Vector3 position`（三维向量 位置）(偏移: 0x4)
...
- `string name`（字符串 名称）(偏移: 0x8)
- `EventTags tags`（事件标签 tags）(偏移: 0xC)
