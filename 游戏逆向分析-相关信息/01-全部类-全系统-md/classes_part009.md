...
- `float length`（浮点数 长度）(偏移: 0x48)
- `float sqrMag`（float sqrMag）(偏移: 0x4C)
- `Vector3 axis`（三维向量 轴）(偏移: 0x50)
...
- `Vector3 offset`（三维向量 偏移）(偏移: 0x54)
...
- `Quaternion IKRotation`（四元数 IK旋转）(偏移: 0x38)
...
- `Vector3 offset`（三维向量 偏移）(偏移: 0x8C)
...
- `Transform target`（变换 目标）(偏移: 0x34)
...
- `Quaternion IKRotation`（四元数 IK旋转）(偏移: 0x3C)
...
- `Transform target`（变换 目标）(偏移: 0x3C)
- `Transform bendGoal`（变换 bendGoal）(偏移: 0x40)
- `float positionWeight`（浮点数 位置权重）(偏移: 0x44)
- `float rotationWeight`（浮点数 旋转权重）(偏移: 0x48)
...
- `Quaternion IKRotation`（四元数 IK旋转）(偏移: 0x8C)
...
- `Quaternion IKRotation`（四元数 IK旋转）(偏移: 0x6C)
...
- `Vector3 offset`（三维向量 偏移）(偏移: 0x44)
...
- `float length`（浮点数 长度）(偏移: 0x40)
- `float sqrMag`（float sqrMag）(偏移: 0x44)
- `Vector3 axis`（三维向量 轴）(偏移: 0x48)
...
- `Vector3 direction`（三维向量 方向）(偏移: 0x38)
- `Vector3 lastPosition`（三维向量 最后位置）(偏移: 0x44)
...
- `string displayName`（字符串 显示名称）(偏移: 0x8)
...
- `bool initiated`（布尔值 已启动）(偏移: 0x88)
...
- `Encoding encoding`（编码 encoding）(偏移: 0xC)
