...
- `Context _context`（上下文 _context）(偏移: 0xC)
...
- `Context _context`（上下文 _context）(偏移: 0x8)
...
- `bool showAlpha`（布尔值 显示透明度）(偏移: 0x8)
- `bool hdr`（布尔值 hdr）(偏移: 0x9)
...
- `int m_Collider`（整数 m_碰撞器）(偏移: 0x24)
