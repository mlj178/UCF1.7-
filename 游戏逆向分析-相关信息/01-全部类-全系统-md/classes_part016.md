...
- `BinaryHeaderEnum binaryHeaderEnum`（二进制头枚举 binary头枚举）(偏移: 0x10)
...
- `FullBodyBipedIK ik`（全身双足IK ik）(偏移: 0x14)
...
- `Quaternion rotation`（四元数 旋转）(偏移: 0x14)
...
- `DrivenRectTransformTracker m_Tracker`（驱动矩形变换跟踪器 m_跟踪器）(偏移: 0xEC)
...
- `int _size`（整数 _大小）(偏移: 0x10)
- `int version`（整数 版本）(偏移: 0x14)
- `IComparer comparer`（I比较器 comparer）(偏移: 0x18)
...
- `FalloffType falloff`（衰减类型 falloff）(偏移: 0x54)
...
- `Vector3 m_Position`（三维向量 m_位置）(偏移: 0x4)
- `Quaternion m_Rotation`（四元数 m_旋转）(偏移: 0x10)
