...
- `bool IsValid(VisualEffect component)`
  （布尔值 是否有效（视觉特效 component））
- `void UpdateTexture()`
  （void 更新纹理（））
- `void UpdateBinding(VisualEffect component)`
  （void 更新绑定（视觉特效 component））
- `string ToString()`
  （字符串 转字符串（））
...
## VersionedMonoBehaviour（版本化MonoBehaviour）
