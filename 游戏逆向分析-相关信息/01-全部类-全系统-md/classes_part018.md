...
- `string displayName`（字符串 显示名称）(偏移: 0x14)
...
- `Player owner`（玩家 所有者）(偏移: 0x48)
- `GameObject effect`（游戏对象 特效）(偏移: 0x4C)
...
- `EventTags tags`（事件标签 tags）(偏移: 0x10)
...
- `EventTags tags`（事件标签 tags）(偏移: 0x20)
...
## TypeDescriptionProvider（类型描述提供者）
...
- `string _typeName`（字符串 _类型名称）(偏移: 0x44)
