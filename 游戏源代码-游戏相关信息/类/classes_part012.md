...
- `float min`（float min）(偏移: 0x14)
- `float max`（float max）(偏移: 0x18)
...
- `string name`（string name）(偏移: 0x24)
...
- `StreamingContext m_context`（StreamingContext m_context）(偏移: 0x2C)
...
- `string[] memberNames`（string[] memberNames）(偏移: 0x1C)
...
- `int objectId`（int objectId）(偏移: 0x2C)
...
- `int objectId`（int objectId）(偏移: 0x8)
- `int numMembers`（int numMembers）(偏移: 0xC)
- `string[] memberNames`（string[] memberNames）(偏移: 0x10)
...
- `string name`（string name）(偏移: 0x18)
...
- `string[] memberNames`（string[] memberNames）(偏移: 0x48)
...
- `StreamingContext m_context`（StreamingContext m_context）(偏移: 0x18)
...
- `bool inited`（bool inited）(偏移: 0x8)
- `bool fakeValue`（bool fake值）(偏移: 0x9)
- `bool fakeValueActive`（bool fake值激活的）(偏移: 0xA)
...
- `bool inited`（bool inited）(偏移: 0x0)
- `byte fakeValue`（byte fake值）(偏移: 0x0)
- `bool fakeValueActive`（bool fake值激活的）(偏移: 0x0)
...
- `bool inited`（bool inited）(偏移: 0x4)
- `char fakeValue`（char fake值）(偏移: 0x6)
- `bool fakeValueActive`（bool fake值激活的）(偏移: 0x8)
...
- `bool inited`（bool inited）(偏移: 0x18)
- `Decimal fakeValue`（Decimal fake值）(偏移: 0x1C)
- `bool fakeValueActive`（bool fake值激活的）(偏移: 0x2C)
...
- `bool fakeValueActive`（bool fake值激活的）(偏移: 0x28)
...
- `int cryptoKey`（int crypto键）(偏移: 0x0)
- `int currentCryptoKey`（int currentCrypto键）(偏移: 0x0)
- `int hiddenValue`（int hidden值）(偏移: 0x4)
- `ACTkByte4 hiddenValueOldByte4`（ACTkByte4 hidden值旧的Byte4）(偏移: 0x8)
- `bool inited`（bool inited）(偏移: 0xC)
- `float fakeValue`（float fake值）(偏移: 0x10)
- `bool fakeValueActive`（bool fake值激活的）(偏移: 0x14)

### 方法 (28)

- `void SetNewCryptoKey(int newKey)`
  （void 集合新的Crypto键（int newKey））
...
- `bool fakeValueActive`（bool fake值激活的）(偏移: 0x10)
...
- `long currentCryptoKey`（long currentCrypto键）(偏移: 0x0)
- `long hiddenValue`（long hidden值）(偏移: 0x8)
- `bool inited`（bool inited）(偏移: 0x10)
- `long fakeValue`（long fake值）(偏移: 0x18)
- `bool fakeValueActive`（bool fake值激活的）(偏移: 0x20)
...
- `bool fakeValueActive`（bool fake值激活的）(偏移: 0x18)
...
- `Vector3 position`（三维向量 position）(偏移: 0xC)
- `Vector2 dir`（二维向量 dir）(偏移: 0x18)
- `float height`（float height）(偏移: 0x20)
...
- `FullBodyBipedIK ik`（满身体BipedIK ik）(偏移: 0x10)
- `float lastTime`（float last时间）(偏移: 0x14)
...
- `Stream _innerStream`（流 _inner流）(偏移: 0x20)
...
- `bool _leaveOpen`（bool _leave打开）(偏移: 0x18)
...
- `float duration`（float duration）(偏移: 0x34)
...
- `Seeker seeker`（Seeker seeker）(偏移: 0x8)
...
- `GraphNode node`（Graph节点 node）(偏移: 0x8)
...
- `float delay`（float delay）(偏移: 0x14)
- `int index`（int index）(偏移: 0x18)
...
- `Vector3 offset`（三维向量 offset）(偏移: 0x24)
...
- `float EndInvoke(IAsyncResult result)`
  （float 结束Invoke（I异步Result result））
