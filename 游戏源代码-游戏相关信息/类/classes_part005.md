...
- `bool m_isReadOnly`（bool m_isReadOnly）(偏移: 0x8)
...
- `string m_name`（string m_name）(偏移: 0x30)
...
- `object value`（object value）(偏移: 0x4)
...
## CustomYieldInstruction（自定义的YieldInstruction）
...
**继承**: CustomYieldInstruction（自定义的YieldInstruction）

### 字段 (1)

- `Tween t`（Tween t）(偏移: 0x8)
...
- `bool m_disposed`（bool m_disposed）(偏移: 0x0)
...
- `bool m_disposed`（bool m_disposed）(偏移: 0x11)
...
- `int index`（int index）(偏移: 0x4)
- `int length`（int length）(偏移: 0x8)
- `DTSubStringType type`（DT子字符串类型 type）(偏移: 0xC)
- `int value`（int value）(偏移: 0x10)
...
- `string m_name`（string m_name）(偏移: 0xC)
- `string m_langName`（string m_lang名称）(偏移: 0x10)
- `CompareInfo m_compareInfo`（Compare信息 m_compare信息）(偏移: 0x14)
...
- `bool m_isReadOnly`（bool m_isReadOnly）(偏移: 0xA4)
...
- `bool hdr`（bool hdr）(偏移: 0x28)
- `bool showAlpha`（bool show透明度）(偏移: 0x29)
- `bool showPicker`（bool showPicker）(偏移: 0x2A)
- `float incStep`（float incStep）(偏移: 0x2C)
- `float incStepMult`（float incStepMult）(偏移: 0x30)
- `int decimals`（int decimals）(偏移: 0x34)
...
- `float incStep`（float incStep）(偏移: 0x30)
- `float incStepMult`（float incStepMult）(偏移: 0x34)
- `int decimals`（int decimals）(偏移: 0x38)
...
- `float incStep`（float incStep）(偏移: 0x28)
- `float incStepMult`（float incStepMult）(偏移: 0x2C)
- `int decimals`（int decimals）(偏移: 0x30)
...
**继承**: DebugUIHandlerWidget（Debug界面处理器Widget）

### 字段 (5)

- `Text nameLabel`（文本 name标签）(偏移: 0x3C)
- `UIFoldout valueToggle`（界面Foldout value开关）(偏移: 0x40)
...
- `Text nameLabel`（文本 name标签）(偏移: 0x3C)
- `Text valueLabel`（文本 value标签）(偏移: 0x40)
...
- `Text nameLabel`（文本 name标签）(偏移: 0xC)
...
## DebugUIHandlerWidget（Debug界面处理器Widget）
...
- `object s_InternalSyncObject`（object s_内部的同步对象）(偏移: 0x8)
...
## DecoderFallbackBuffer（DecoderFallback缓冲区）
