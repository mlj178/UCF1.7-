...
- `FullBodyBipedIK ik`（满身体BipedIK ik）(偏移: 0xC)
...
- `float rotationWeight`（float rotationWeight）(偏移: 0x20)
...
- `Vector3 offset`（三维向量 offset）(偏移: 0x70)
...
- `Quaternion defaultLocalRotation`（Quaternion default本地的Rotation）(偏移: 0x10)
...
- `bool initiated`（bool initiated）(偏移: 0x38)
- `float length`（float length）(偏移: 0x3C)
- `float distance`（float distance）(偏移: 0x40)
...
- `ClampedFloatParameter intensity`（Clamped浮点数Parameter intensity）(偏移: 0x20)
...
- `float rotationWeight`（float rotationWeight）(偏移: 0xC)
...
- `Transform target`（变换 target）(偏移: 0x28)
...
- `Vector3 startPoint`（三维向量 startPoint）(偏移: 0x84)
...
- `Vector3 startPoint`（三维向量 startPoint）(偏移: 0x8)
- `Vector3 endPoint`（三维向量 endPoint）(偏移: 0x14)
...
- `ProfilingSampler m_ProfilingSampler`（ProfilingSampler m_ProfilingSampler）(偏移: 0x54)
...
- `FilteringSettings m_FilteringSettings`（FilteringSettings m_FilteringSettings）(偏移: 0x64)
...
- `string m_Name`（string m_名称）(偏移: 0x3C)
...
- `bool _disposed`（bool _disposed）(偏移: 0x30)
...
- `Transform root`（变换 root）(偏移: 0x48)
...
- `Vector3 defaultLocalPosition`（三维向量 default本地的Position）(偏移: 0x10)
- `Quaternion defaultLocalRotation`（Quaternion default本地的Rotation）(偏移: 0x1C)
