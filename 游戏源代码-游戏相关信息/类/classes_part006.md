...
- `Stream _innerStream`（流 _inner流）(偏移: 0x18)
- `bool _disposed`（bool _disposed）(偏移: 0x1C)
...
- `FilteringSettings m_FilteringSettings`（FilteringSettings m_FilteringSettings）(偏移: 0x100)
...
- `FilteringSettings m_FilteringSettings`（FilteringSettings m_FilteringSettings）(偏移: 0xAC)
...
- `Vector3 direction`（三维向量 direction）(偏移: 0x48)
...
- `float radius`（float radius）(偏移: 0x48)
...
- `FilteringSettings m_FilteringSettings`（FilteringSettings m_FilteringSettings）(偏移: 0x54)
...
- `ProfilingSampler m_ProfilingSampler`（ProfilingSampler m_ProfilingSampler）(偏移: 0xE0)
...
- `Transform root`（变换 root）(偏移: 0x44)
...
- `bool m_isReadOnly`（bool m_isReadOnly）(偏移: 0x11)
...
- `object s_InternalSyncObject`（object s_内部的同步对象）(偏移: 0x24)
...
- `IComparer comparer`（IComparer comparer）(偏移: 0x10)
...
- `bool m_disposed`（bool m_disposed）(偏移: 0x48)
...
- `EventTags tags`（事件Tags tags）(偏移: 0x8)
