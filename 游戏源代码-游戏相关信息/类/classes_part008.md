...
- `float height`（float height）(偏移: 0x10)
...
- `GraphNode node`（Graph节点 node）(偏移: 0x18)
...
- `Quaternion rotation`（Quaternion rotation）(偏移: 0xB0)
...
- `Bounds bounds`（Bounds bounds）(偏移: 0x8)
...
- `int m_Version`（int m_Version）(偏移: 0x4)
...
- `int width`（int width）(偏移: 0xBC)
- `int depth`（int depth）(偏移: 0xC0)
...
- `Vector3 rotation`（三维向量 rotation）(偏移: 0xD0)
...
- `FullBodyBipedIK ik`（满身体BipedIK ik）(偏移: 0x20)
- `float spineBend`（float spineBend）(偏移: 0x24)
- `float spineSpeed`（float spineSpeed）(偏移: 0x28)
...
- `Transform pelvis`（变换 pelvis）(偏移: 0x24)
...
- `Transform pelvis`（变换 pelvis）(偏移: 0x44)
- `Transform lastSpineBone`（变换 lastSpineBone）(偏移: 0x48)
- `Transform head`（变换 head）(偏移: 0x4C)
...
- `Quaternion rotation`（Quaternion rotation）(偏移: 0x8)
...
- `bool initiated`（bool initiated）(偏移: 0x8C)
...
- `float lastTime`（float last时间）(偏移: 0xA4)
- `float deltaTime`（float delta时间）(偏移: 0xA8)
- `Vector3 lastPosition`（三维向量 lastPosition）(偏移: 0xAC)
...
- `float lastTime`（float last时间）(偏移: 0x30)
...
- `void PlayerController_FocusHUD_Listenner(ref string hud)`
  （void 玩家Controller_聚焦HUD_Listenner（ref string hud））
- `void Update()`
  （void 更新（））
...
- `float height`（float height）(偏移: 0x4)
...
- `RectTransform rect`（Rect变换 rect）(偏移: 0x30)
...
**继承**: HUD_ProjectionSign（HUD_Projection标志）
...
- `RectTransform rect`（Rect变换 rect）(偏移: 0x14)
- `Image image`（图像 image）(偏移: 0x18)
...
## HUD_ProjectionSign（HUD_Projection标志）
...
- `Image image`（图像 image）(偏移: 0x30)
- `RectTransform rect`（Rect变换 rect）(偏移: 0x34)
...
- `RectTransform rect`（Rect变换 rect）(偏移: 0x10)
...
- `int version`（int version）(偏移: 0x1C)
...
- `object _syncRoot`（object _sync根）(偏移: 0x30)
...
- `Vector3 velocity`（三维向量 velocity）(偏移: 0x0)
- `float weight`（float weight）(偏移: 0xC)
- `float time`（float time）(偏移: 0x10)
