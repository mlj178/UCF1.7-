...
- `Vector3 velocity`（三维向量 velocity）(偏移: 0x260)
...
- `bool m_disposed`（bool m_disposed）(偏移: 0x18)
...
- `Vector3 offset`（三维向量 offset）(偏移: 0x4)
- `Vector3 euler`（三维向量 euler）(偏移: 0x10)
...
- `ClampedFloatParameter intensity`（Clamped浮点数Parameter intensity）(偏移: 0x1C)
...
**继承**: CinemachineComponentBase（Cinemachine组件基础）
...
- `void MutateCameraState(ref CameraState curState, float deltaTime)`
  （void Mutate摄像机状态（ref CameraState curState, float deltaTime））
...
**继承**: CinemachineVirtualCameraBase（Cinemachine虚拟的摄像机基础）

### 字段 (10)

- `Transform m_LookAt`（变换 m_LookAt）(偏移: 0x40)
- `Transform m_Follow`（变换 m_Follow）(偏移: 0x44)
- `bool m_ShowDebugText`（bool m_显示Debug文本）(偏移: 0x48)
- `bool m_Loop`（bool m_Loop）(偏移: 0x49)
- `CinemachineVirtualCameraBase[] m_ChildCameras`（Cinemachine虚拟的摄像机Base[] m_子级Cameras）(偏移: 0x4C)
...
- `void set_LiveChild(ICinemachineCamera value)`
  （void set_Live子级（ICinemachine摄像机 value））
- `ICinemachineCamera get_LiveChild()`
  （ICinemachine摄像机 get_Live子级（））
...
- `void ForceCameraPosition(Vector3 pos, Quaternion rot)`
  （void 强制摄像机Position（三维向量 pos, Quaternion rot））
...
- `void OnTransformChildrenChanged()`
  （void On变换ChildrenChanged（））
- `void OnGuiHandler()`
  （void OnGui处理器（））
- `CinemachineVirtualCameraBase[] get_ChildCameras()`
  （Cinemachine虚拟的摄像机Base[] get_子级Cameras（））
- `bool get_IsBlending()`
  （bool get_是否Blending（））
- `void InvalidateListOfChildren()`
  （void Invalidate列表OfChildren（））
...
- `bool m_ShowDebugText`（bool m_显示Debug文本）(偏移: 0xC)
...
- `float m_Damping`（float m_Damping）(偏移: 0x40)
...
## CinemachineComponentBase（Cinemachine组件基础）
...
- `bool OnTransitionFromCamera(ICinemachineCamera fromCam, Vector3 worldUp, float deltaTime, ref CinemachineVirtualCameraBase.TransitionParams transitionParams)`
  （bool OnTransitionFrom摄像机（ICinemachine摄像机 fromCam, 三维向量 worldUp, float deltaTime, ref CinemachineVirtualCameraBase.TransitionParams transitionParams））
- `void OnTargetObjectWarped(Transform target, Vector3 positionDelta)`
  （void On目标对象Warped（变换 target, 三维向量 positionDelta））
- `void ForceCameraPosition(Vector3 pos, Quaternion rot)`
  （void 强制摄像机Position（三维向量 pos, Quaternion rot））
...
- `float m_Damping`（float m_Damping）(偏移: 0x28)
...
- `float m_Damping`（float m_Damping）(偏移: 0x18)
...
- `Vector3 m_Position`（三维向量 m_Position）(偏移: 0x28)
...
- `CinemachineVirtualCameraBase[] m_ChildCameras`（Cinemachine虚拟的摄像机Base[] m_子级Cameras）(偏移: 0x124)
...
- `float width`（float width）(偏移: 0x28)
...
- `bool m_ShowDebugText`（bool m_显示Debug文本）(偏移: 0x50)
- `CinemachineVirtualCameraBase[] m_ChildCameras`（Cinemachine虚拟的摄像机Base[] m_子级Cameras）(偏移: 0x54)
...
- `Transform target`（变换 target）(偏移: 0x0)
- `float weight`（float weight）(偏移: 0x4)
...
## CinemachineVirtualCameraBase（Cinemachine虚拟的摄像机基础）
