...
- `int priority`（int priority）(偏移: 0x8C)
...
- `int version`（int version）(偏移: 0x4)
...
- `float m_Value`（float m_值）(偏移: 0x4)
...
- `float range`（float range）(偏移: 0x4)
...
- `float height`（float height）(偏移: 0x38)
...
- `bool enableRotation`（bool enableRotation）(偏移: 0x58)
- `float rotationSpeed`（float rotationSpeed）(偏移: 0x5C)
...
- `Vector3 velocity`（三维向量 velocity）(偏移: 0x12C)
...
- `LinearColor color`（Linear颜色 color）(偏移: 0xC)
- `LinearColor indirectColor`（Linear颜色 indirect颜色）(偏移: 0x1C)
- `Quaternion orientation`（Quaternion orientation）(偏移: 0x2C)
- `Vector3 position`（三维向量 position）(偏移: 0x3C)
- `float range`（float range）(偏移: 0x48)
...
- `FalloffType falloff`（Falloff类型 falloff）(偏移: 0x5F)
...
- `IComparer comparer`（IComparer comparer）(偏移: 0x14)
- `object _syncRoot`（object _sync根）(偏移: 0x18)
...
- `float weight`（float weight）(偏移: 0x14)
- `Vector3 offset`（三维向量 offset）(偏移: 0x18)
...
- `Vector3 lastPosition`（三维向量 lastPosition）(偏移: 0x60)
...
- `float duration`（float duration）(偏移: 0xC)
...
- `float width`（float width）(偏移: 0x14)
- `float height`（float height）(偏移: 0x18)
...
- `Hashtable _caps`（Hashtable _caps）(偏移: 0x44)
