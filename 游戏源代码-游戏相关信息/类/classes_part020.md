...
- `int width`（int width）(偏移: 0x8)
- `int depth`（int depth）(偏移: 0xC)
...
- `Bounds bounds`（Bounds bounds）(偏移: 0xC)
...
- `int width`（int width）(偏移: 0x58)
- `int depth`（int depth）(偏移: 0x5C)
...
- `Skill_Common skill`（Skill_Common skill）(偏移: 0xF0)
- `GameObject skillBtn`（游戏对象 skillBtn）(偏移: 0xF4)
- `GameObject effect`（游戏对象 effect）(偏移: 0xF8)
...
- `Skill_Common skill`（Skill_Common skill）(偏移: 0xF4)
...
- `Vector3 startPoint`（三维向量 startPoint）(偏移: 0xF4)
- `Vector3 endPoint`（三维向量 endPoint）(偏移: 0x100)
...
- `Transform bone`（变换 bone）(偏移: 0x0)
- `float distance`（float distance）(偏移: 0x4)
...
- `float range`（float range）(偏移: 0x148)
...
- `Vector3 m_Position`（三维向量 m_Position）(偏移: 0x8)
- `Quaternion m_Rotation`（Quaternion m_Rotation）(偏移: 0x14)
...
- `float width`（float width）(偏移: 0x0)
...
- `bool _disposed`（bool _disposed）(偏移: 0x34)
...
## ZipProgressEventArgs（ZipProgress事件Args）
...
- `Stream _innerStream`（流 _inner流）(偏移: 0x38)
...
- `bool _leaveOpen`（bool _leave打开）(偏移: 0x2C)
