...
- `Vector3 startPoint`（三维向量 startPoint）(偏移: 0x98)
- `Vector3 endPoint`（三维向量 endPoint）(偏移: 0xA4)
...
## ACTkDetectorBase（ACTkDetector基础）
...
- `float height`（float height）(偏移: 0x14)
- `float repathRate`（float repathRate）(偏移: 0x18)
...
- `bool enableRotation`（bool enableRotation）(偏移: 0x3C)
- `Vector3 simulatedPosition`（三维向量 simulatedPosition）(偏移: 0x40)
- `Quaternion simulatedRotation`（Quaternion simulatedRotation）(偏移: 0x4C)
...
- `Seeker seeker`（Seeker seeker）(偏移: 0x74)
...
- `bool updatePosition`（bool updatePosition）(偏移: 0x90)
- `bool updateRotation`（bool updateRotation）(偏移: 0x91)
- `float lastDeltaTime`（float lastDelta时间）(偏移: 0x94)
- `int prevFrame`（int prevFrame）(偏移: 0x98)
...
- `float lastRepath`（float lastRepath）(偏移: 0xC0)
...
- `Transform target`（变换 target）(偏移: 0x10)
...
- `float repathRate`（float repathRate）(偏移: 0x10)
- `bool canSearch`（bool can搜索）(偏移: 0x14)
- `bool canMove`（bool can移动）(偏移: 0x15)
- `float speed`（float speed）(偏移: 0x18)
- `OrientationMode orientation`（Orientation模式 orientation）(偏移: 0x1C)
- `bool enableRotation`（bool enableRotation）(偏移: 0x20)
- `float rotationSpeed`（float rotationSpeed）(偏移: 0x24)
...
- `bool updatePosition`（bool updatePosition）(偏移: 0x40)
- `bool updateRotation`（bool updateRotation）(偏移: 0x41)
- `Seeker seeker`（Seeker seeker）(偏移: 0x48)
- `Transform tr`（变换 tr）(偏移: 0x4C)
- `float lastRepath`（float lastRepath）(偏移: 0x50)
...
- `Quaternion simulatedRotation`（Quaternion simulatedRotation）(偏移: 0xA4)
...
- `bool alwaysDrawGizmos`（bool alwaysDrawGizmos）(偏移: 0xF4)
...
- `bool constrainInsideGraph`（bool constrainInsideGraph）(偏移: 0xFC)
- `Path path`（路径 path）(偏移: 0x100)
- `PathInterpolator interpolator`（路径Interpolator interpolator）(偏移: 0x104)
- `NNConstraint cachedNNConstraint`（NNConstraint cachedNNConstraint）(偏移: 0x0)
...
- `float length`（float length）(偏移: 0x0)
- `int id`（int id）(偏移: 0x4)
...
- `float radius`（float radius）(偏移: 0x8)
- `float height`（float height）(偏移: 0xC)
...
- `float radius`（float radius）(偏移: 0x3C)
...
- `float weight`（float weight）(偏移: 0x10)
...
- `Vector3 offset`（三维向量 offset）(偏移: 0x40)
...
- `Vector3 lastPosition`（三维向量 lastPosition）(偏移: 0x78)
...
- `Vector3 direction`（三维向量 direction）(偏移: 0x10)
...
- `object value`（object value）(偏移: 0x8)
...
- `float speed`（float speed）(偏移: 0x1C)
...
- `Vector3 m_Position`（三维向量 m_Position）(偏移: 0x10)
...
- `int m_Version`（int m_Version）(偏移: 0x3C)
- `Quaternion m_Rotation`（Quaternion m_Rotation）(偏移: 0x40)
...
- `Vector3 m_Position`（三维向量 m_Position）(偏移: 0x8C)
...
- `Quaternion m_Rotation`（Quaternion m_Rotation）(偏移: 0xC4)
...
- `IComparer comparer`（IComparer comparer）(偏移: 0x8)
...
- `int version`（int version）(偏移: 0x10)
...
- `void CopyFrom(AsnEncodedData asnEncodedData)`
  （void 复制From（AsnEncoded数据 asnEncodedData））
- `string ToString(bool multiLine)`
  （string To字符串（bool multiLine））
...
- `DrivenRectTransformTracker m_Tracker`（DrivenRect变换Tracker m_Tracker）(偏移: 0x1A)
...
- `float distance`（float distance）(偏移: 0x10)
- `float height`（float height）(偏移: 0x14)
