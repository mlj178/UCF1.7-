...
def _control_default(manifest, key):
...
class MigratedOrdinaryFeatureTests(unittest.TestCase):
    ...
    def test_no_manual_ordinary_imports_or_central_panel_branches(self):
        ...
        self.assertIn("name='UCF2.1修改器'", spec_text)
        for feature_id in self.ORDINARY_FEATURES:
    ...
    def test_requested_compact_card_titles_and_free_camera_layout(self):
        godmode_manifest = json.loads((feature_dir("godmode") / "manifest.json").read_text(encoding="utf-8"))
        free_camera_manifest = json.loads((feature_dir("free_camera") / "manifest.json").read_text(encoding="utf-8"))
        feature_card = (PROJECT_DIR / "ui" / "components" / "feature_card.py").read_text(encoding="utf-8")
        free_panel = (feature_dir("free_camera") / "panel.py").read_text(encoding="utf-8")

        self.assertEqual(godmode_manifest["layout"].get("title_joiner"), "")
        self.assertIn("title_icon_label", feature_card)
        self.assertIn("title_name_label", feature_card)
        self.assertIn("title_icon_label", free_panel)
        self.assertIn("title_name_label", free_panel)
        self.assertNotIn('text=f"{manifest[\'icon\']}{manifest[\'display_name\']}"', free_panel)
        self.assertEqual(free_camera_manifest["layout"]["columnspan"], 1)
        self.assertEqual(free_camera_manifest["layout"]["card_type"], "simple")
        self.assertEqual(free_camera_manifest["config"]["moveSpeed"], 30.0)
        self.assertEqual(_control_default(free_camera_manifest, "moveSpeed"), 30.0)
        self.assertEqual(free_camera_manifest["config"]["lockPlayerMove"], True)
        self.assertNotIn("lockPlayerMove", [item.get("key") for item in free_camera_manifest["controls"]])
        self.assertNotIn("CTkCheckBox", free_panel)
        self.assertNotIn('"lockPlayerMove"', free_panel)
    ...
    def test_bot_pause_and_free_camera_formal_integration_contract(self):
        expected = {
            "bot_pause": {
                "display_name": "人机暂停",
                "icon": "⏸️",
                ...
                "tab_title": "其他",
                "config": {},
                "rpc": ["enable", "disable", "setConfig", "setconfig", "status", "cleanup"],
                "handles": ["bot_pause_switch"],
                "script_needles": [
                    "Bot_TypeInfo:    GameAssembly.dll + 0xE25F44",
                    "Resolve pointers on every operation; never keep stale pointers.",
                    "Bot.stopAllBot",
                    "GameManager_Awake",
                    "ModeBase_Update",
                    "Bot_Update",
            ...
            "free_camera": {
                ...
                "rpc": ["enable", "disable", "setConfig", "setconfig", "configure", "status", "cleanup"],
                "handles": ["free_camera_switch", "free_camera_move_speed_var"],
                "script_needles": [
                    "free_camera_v1_3.js",
                ...
                "panel_needles": [
                    "callbacks[\"toggle\"](feature_id)",
                    "callbacks[\"set_config\"](feature_id, \"moveSpeed\"",
                ],
            },
        }
        ...
        for feature_id, values in expected.items():
            with self.subTest(feature_id=feature_id):
                ...
                events = (plugin_dir / "events.py").read_text(encoding="utf-8")

                self.assertEqual(manifest["feature_id"], feature_id)
                self.assertEqual(manifest["display_name"], values["display_name"])
                self.assertEqual(manifest["icon"], values["icon"])
                self.assertEqual(manifest["desc"], values["desc"])
                self.assertEqual(manifest["category"], values["category"])
                self.assertEqual(manifest["tab"], values["tab"])
                self.assertEqual(manifest["tab_title"], values["tab_title"])
                self.assertEqual(manifest["config"], values["config"])
                self.assertEqual(manifest["runtime"], {"type": "plugin_script"})
                self.assertEqual(manifest["state"], {"sync_enabled_from_config": True})
                self.assertIs(manifest["lifecycle"]["cleanup"], True)
                self.assertIs(manifest["lifecycle"]["restore"], True)
                self.assertIs(manifest["lifecycle"]["requires_room_ready_reapply"], True)
                self.assertIs(manifest["lifecycle"]["room_ready_retry"], True)
                self.assertEqual(manifest["controls"][0], {"type": "switch", "action": "enable"})
                self.assertNotIn("hotkey", manifest)
                for action in values["rpc"]:
                    self.assertIn(action, manifest["rpc"])
                for handle in values["handles"]:
                    self.assertIn(handle, manifest["ui_handles"].values())
                ...
                self.assertIn("def handle_event", events)
    ...
    def test_sound_manager_suppresses_pygame_support_prompt(self):
        ...
        self.assertIn("def _resource_file", sound_manager)
        self.assertIn("self._toggle_sound", sound_manager)
        self.assertIn("sound.set_volume", sound_manager)
        self.assertIn("def _play_with_pygame_music", sound_manager)
        self.assertIn("winsound.MessageBeep", sound_manager)
        self.assertIn("if result is not False:", app_text)
        self.assertNotIn("self._feature_controller.toggle_feature(feature_id)\n            self._sound.play_toggle_sound()", app_text)
