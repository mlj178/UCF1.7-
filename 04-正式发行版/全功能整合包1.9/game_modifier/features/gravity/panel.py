...
def build_card(parent, manifest, row, col, colspan, callbacks, card_builder):
    ...
    gravity_slider = ctk.CTkSlider(slider_frame, from_=0.0, to=1.0, variable=gravity_var, number_of_steps=10, command=callbacks["gravity"], width=100)
    ...
    jump_slider = ctk.CTkSlider(slider_frame, from_=1.0, to=5.0, variable=jump_var, number_of_steps=8, command=callbacks["jump"], width=100)
    ...
    mode_combo = ctk.CTkComboBox(mode_frame, values=list(GRAVITY_MODE_VALUES), variable=mode_var, font=("Microsoft YaHei", 12), height=30, width=120, state="readonly", command=callbacks["gravity_mode"])
