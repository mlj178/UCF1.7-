...
class AppPersistenceService:
    ...
    def load_app_state(self, feature_ids, restorable_feature_ids=None):
        state = AppState.with_default_features(feature_ids)
        state.apply_feature_state_payload(
            self._feature_state_repo.load(),
            allowed_feature_ids=restorable_feature_ids,
        )
        state.apply_desired_states(self.load_desired_states())
        return state

    def save_app_state(self, state, restorable_feature_ids=None):
        self._feature_state_repo.save(
            state.to_feature_state_payload(
                skip_feature_ids=self.DESIRED_FEATURE_IDS,
                allowed_feature_ids=restorable_feature_ids,
            )
        )
