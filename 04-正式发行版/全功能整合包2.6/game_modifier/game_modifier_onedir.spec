...
# 正式版版本号只允许来自上一级整合包目录，例如“全功能整合包2.4”。
...
if os.path.exists(ui_dir):
    for root, dirs, files in os.walk(ui_dir):
        ...
        for file in files:
            if file.endswith((".py", ".pyc", ".pyo")) or file.startswith("test_") or file.endswith("_test"):
                continue
...
# onedir：EXE 不含 binaries/datas，改由 COLLECT 收集（安装器打包规范 §6.3）
exe = EXE(
    pyz,
    a.scripts,
    [],
    ...
    uac_admin=True, # 主 exe 每次启动弹 UAC，见安装器打包规范 §9.1
    icon=None,
)
