...
class AppState:
    ...
    def apply_feature_state_payload(self, payload, allowed_feature_ids=None):
        allowed_feature_ids = set(allowed_feature_ids or self.features.keys())
        for fid, config in payload.items():
            if fid not in allowed_feature_ids:
                continue
    ...
    def to_feature_state_payload(self, skip_feature_ids=None, allowed_feature_ids=None):
        skip_feature_ids = set(skip_feature_ids or ())
        allowed_feature_ids = set(allowed_feature_ids or self.features.keys())
        payload = {}
        for fid, enabled in self.features.items():
            if fid in skip_feature_ids or fid not in allowed_feature_ids:
                continue
