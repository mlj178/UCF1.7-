...
class Feature30And31PluginIntegrationTests(unittest.TestCase):
    ...
    def test_fixed_point_teleport_manifest_uses_stable_id_and_point_actions(self):
        ...
        self.assertEqual(manifest["runtime"]["type"], "plugin_script")
        self.assertTrue(manifest["state"]["sync_enabled_from_config"])
        for rpc_name in ("savepoint", "teleporttopoint", "savepoint2", "teleporttopoint2", "setConfig", "set_config", "setconfig"):
            self.assertIn(rpc_name, manifest["rpc"])
        for action_name in ("savepoint", "teleporttopoint", "savepoint2", "teleporttopoint2"):
        ...
        self.assertEqual(controls_by_action["savepoint"]["label"], "保存点位1  Alt+1")
        self.assertEqual(controls_by_action["teleporttopoint"]["label"], "瞬移到点位1  Alt+2")
        self.assertEqual(controls_by_action["savepoint2"]["label"], "保存点位2  Alt+3")
        self.assertEqual(controls_by_action["teleporttopoint2"]["label"], "瞬移到点位2  Alt+4")
        ...
        self.assertIn("点位1", panel_text)
        self.assertIn("点位2", panel_text)
        self.assertIn("快捷键：Alt+1 保存点位1，Alt+2 瞬移到点位1；Alt+3 保存点位2，Alt+4 瞬移到点位2", panel_text)
    ...
    def test_alt_1_to_alt_4_are_dedicated_fixed_point_teleport_actions(self):
        ...
        self.assertRegex(config_text, r"'alt\+3':\s*\{[^}]*'feature_id':\s*'fixed_point_teleport'[^}]*'action':\s*'savepoint2'")
        self.assertRegex(config_text, r"'alt\+4':\s*\{[^}]*'feature_id':\s*'fixed_point_teleport'[^}]*'action':\s*'teleporttopoint2'")
