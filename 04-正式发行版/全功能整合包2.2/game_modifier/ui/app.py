...
class App(ctk.CTk):
    def __init__(self):
        ...
        self._persistent_state = self._persistence_service.load_app_state(self._feature_ids)
    ...
    def _load_feature_state(self):
        try:
            state = self._persistence_service.load_app_state(self._feature_ids)
    ...
    def _migrate_plugin_config_from_persistent_state(self):
        ...
        # provide defaults in manifest/config files and should not be added here.
        state = self._persistent_state
        for feature in self._plugin_registry.all():
            fid = feature.manifest["feature_id"]
            if self._config_manager.has_user_config(fid):
                continue
            payload = {"enabled": self._features.get(fid, False)}
