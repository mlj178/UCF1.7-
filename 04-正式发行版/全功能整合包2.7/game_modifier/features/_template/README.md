...
- 如果功能依赖玩家、Bot、GameManager、相机、武器等进房后才存在的运行态对象，`manifest.lifecycle.requires_room_ready_reapply` 和 `room_ready_retry` 必须为 `true`，并在 JS 中实现 `pendingApply/applied/lastApplyReason`。
- 如果功能不需要房间就绪后二次应用，必须写 `lifecycle.room_ready_reapply_exempt_reason` 说明原因，例如“只安装固定 RVA Hook，Hook 命中时按 enabled 状态自然生效”。
