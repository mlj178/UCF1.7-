...
class VersionMetadataStaticTests(unittest.TestCase):
    def test_application_version_is_2_5(self):
        self.assertIn('APP_VERSION = "v2.5"', WINDOW_CONTRACT.read_text(encoding="utf-8"))

    def test_about_page_shows_contributor_immediately_after_author(self):
        text = SETTINGS_WINDOW.read_text(encoding="utf-8")
        self.assertIn('text="Contributor(贡献者)：少年与狗子"', text)
        author_index = text.index('text="作者: 挂呱呱呱"')
        contributor_index = text.index('text="Contributor(贡献者)：少年与狗子"')
        date_index = text.index('text="2026年08月08日"')

        self.assertLess(author_index, contributor_index)
        self.assertLess(contributor_index, date_index)

    def test_installer_default_version_is_2_5(self):
        text = INSTALLER_SCRIPT.read_text(encoding="utf-8")
        self.assertIn('#define AppVersion "2.5"', text)
        self.assertIn('版本号默认取 2.5', text)
