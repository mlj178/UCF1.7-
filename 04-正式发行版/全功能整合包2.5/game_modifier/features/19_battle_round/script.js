...
function __battleRoundModule() {
  return modules.battle_round_always;
}

rpc.exports = {
  enable: function(config) {
    var module = __battleRoundModule();
    if (!module || typeof module.enable !== 'function') return { ok: false, msg: '决战回合模块未加载' };
    module.enable();
  ...
  status: function() {
    var module = __battleRoundModule();
    if (!module || typeof module.getStatus !== 'function') return { enabled: false, hookInstalled: false, currentIsBattleRound: -1 };
    return parseJsonResult(module.getStatus(), { enabled: false, hookInstalled: false, currentIsBattleRound: -1 });
  },
  cleanup: function(payload) {
    var module = __battleRoundModule();
    if (module && typeof module.cleanup === 'function') module.cleanup();
    return { ok: true, reason: payload && payload.reason ? payload.reason : 'cleanup' };
  },
  battleRoundGetStatus: function(payload) {
    return rpc.exports.status();
  }
};
