...
(function() {
  ...
  var aimModule = (function() {
    ...
    // 可见性检测：Linecast 检查是否有障碍物
    function isVisible(start, end) {
      try {
        if (!physicsLinecastFn) return true;
        ...
        endBuf.writeFloat(end.x);
        endBuf.add(4).writeFloat(end.y);
        endBuf.add(8).writeFloat(end.z);
        var hit = physicsLinecastFn(startBuf, endBuf, -1, ptr(0));
        return !hit;  // Linecast=true 有障碍 → 不可见; false=无障碍 → 可见
      } catch(e) { return true; }
    }
    ...
    function aimLoop() {
      ...
      if (lockedTarget) {
        try {
          ...
          if (stillDead || !tPos) {
            ...
            // 检查是否仍在 FOV 内
            var dx2 = tPos.x - myPos.x, dy2 = tPos.y - myPos.y, dz2 = tPos.z - myPos.z;
      ...
      // 如果没有锁定目标，选新的
      var best = null;
      var bestAngleDist = Number.MAX_VALUE;
      if (!lockedTarget) {
        for (var i = 0; i < enemies.length; i++) {
          ...
          // 计算角度距离（DLL 方式）
          var dx2 = ePos.x - myPos.x, dy2 = ePos.y - myPos.y, dz2 = ePos.z - myPos.z;
          var tYaw = Math.atan2(dx2, dz2);
          var tDist2d = Math.sqrt(dx2*dx2 + dz2*dz2);
          var tPitch = Math.atan2(dy2, tDist2d);
          var cyRad = curYawDeg * Math.PI / 180;
          ...
          if (yawDif < -Math.PI) yawDif += 2*Math.PI;
          var pitchDif = tPitch - cpRad;
          if (pitchDif > Math.PI) pitchDif -= 2*Math.PI;
          if (pitchDif < -Math.PI) pitchDif += 2*Math.PI;
          // FOV 过滤
          if (Math.abs(yawDif) > maxYawDiff || Math.abs(pitchDif) > maxPitchDiff) continue;
          // 可见性检测
          if (!isVisible(eyePos, ePos)) {
            sendLog('debug', '调试', tag + ' 被障碍物遮挡，跳过');
            continue;
          }
          var angDist = Math.sqrt(yawDif*yawDif + pitchDif*pitchDif);
          sendLog('debug', '调试', tag + ' 角度距离=' + (angDist*180/Math.PI).toFixed(1) + '°');
          if (angDist < bestAngleDist) {
            bestAngleDist = angDist;
            best = enemies[i];
          }
        }
        if (best) {
          lockedTarget = best;
          sendLog('info', '调试', '锁定新目标 @ ' + best);
        }
      } else {
        best = lockedTarget;
      }
      ...
      sendLog('debug', '调试', '目标角度(度): yaw=' + targetYawDeg.toFixed(2) + '° pitch=' + targetPitchDeg.toFixed(2) + '°');
      ...
      try {
        ...
        sendLog('debug', '角度写入', '平滑后: yaw=' + smoothYaw.toFixed(2) + '° pitch=' + smoothPitch.toFixed(2) + '°');
        ...
        sendLog('debug', '角度写入', '写入后: yaw=' + afterYaw.toFixed(2) + '° pitch=' + afterPitch.toFixed(2) + '°');

        logCount++;
        sendLog('info', '自瞄', '写入角度 yaw=' + smoothYaw.toFixed(1) + '° pitch=' + smoothPitch.toFixed(1) + '°');
      } catch(e) {
