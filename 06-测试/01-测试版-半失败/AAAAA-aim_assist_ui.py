...
JS_FILE = os.path.join(BASE_DIR, "AAAAA-aim_assist_v1.js")
...
class AimAssistApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(
            self, text="鼠标左键瞄准 | 距离最近方案 | 仅对准不开枪",
    ...
    def _do_disable(self):
        try:
            self.script.exports.aimdisable()
    ...
    def _do_update_config(self):
        try:
            ...
            bone = self.bone_var.get()
            self.script.exports.aimconfig(smooth, bone)
        except Exception as e:
            pass
    ...
    def _poll_loop(self):
        """后台轮询：每 0.5 秒调用 JS 的 aimstatus() 获取最新状态"""
        while self._polling_active() and self.is_connected and self.script:
            try:
                raw = self.script.exports.aimstatus()
                status = json.loads(raw)
                with self._gs_lock:
                    self.game_state["enabled"] = status.get("enabled", False)
                    self.game_state["target_info"] = status.get("targetEnemy", None) or "无"
                    self.game_state["smoothness"] = status.get("smoothness", 0.3)
                    self.game_state["aim_bone"] = status.get("aimBone", "chest")
