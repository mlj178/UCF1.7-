# free_camera_ui_v1_2.py
# 自由视角 UI v1.2
# 运行时日志自动写入 logs/free_camera_ui_YYYYMMDD.txt。
...
JS_FILE = os.path.join(BASE_DIR, "free_camera_v1_2.js")
...
class FreeCameraApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(self, text="自由视角 v1.2", font=ctk.CTkFont(size=24, weight="bold")).grid(row=0, column=0, padx=18, pady=(16, 2), sticky="ew")
        ctk.CTkLabel(self, text="鼠标控制朝向，W/A/S/D 控制相机水平移动；默认不锁玩家移动。", font=ctk.CTkFont(size=12), text_color="#888").grid(row=1, column=0, padx=18, pady=(0, 8), sticky="ew")
        ...
        ctk.CTkLabel(ctrl_frame, text="功能开关", font=ctk.CTkFont(size=13, weight="bold")).grid(row=0, column=0, padx=(12, 6), pady=12, sticky="w")
        self.feature_switch = ctk.CTkSwitch(ctrl_frame, text="关闭", font=ctk.CTkFont(size=13), command=self.on_toggle_feature, progress_color=COLOR_GREEN, switch_width=50, switch_height=24)
        ...
        ctk.CTkLabel(config_frame, text="移动速度", font=ctk.CTkFont(size=12)).grid(row=0, column=0, padx=12, pady=8, sticky="w")
        self.speed_var = ctk.DoubleVar(value=8.0)
        self.speed_slider = ctk.CTkSlider(config_frame, from_=1.0, to=30.0, number_of_steps=29, variable=self.speed_var, command=self.on_speed_changed)
        ...
        self.lock_move_var = ctk.BooleanVar(value=False)
        self.lock_move_check = ctk.CTkCheckBox(config_frame, text="开启时锁住玩家水平移动", variable=self.lock_move_var, command=self.on_lock_move_changed)
        ...
        ctk.CTkButton(log_header, text="清空", width=50, height=22, font=ctk.CTkFont(size=10), fg_color="#444", hover_color="#555", command=self.clear_log).pack(side="right")
    ...
    def after_safe(self, delay_ms, callback):
        if self._closing:
            return
        try: self.after(delay_ms, callback)
        except Exception: pass
    ...
    def _set_disconnected(self, reason):
        ...
        try:
            if old_script: old_script.unload()
        except Exception: pass
        try:
            if old_session: old_session.detach()
        except Exception: pass
        if not self._closing:
    ...
    def configure_remote(self):
        if not self.script or not self.is_connected:
            return
        config = {"moveSpeed": float(self.speed_var.get()), "lockPlayerMove": bool(self.lock_move_var.get())}
    ...
    def _configure_worker(self, config):
        try:
            with self._rpc_lock:
                script = self.script
                if script:
                    result = script.exports_sync.configure(config)
                    if isinstance(result, dict) and not result.get("ok", True):
                        self._post_ui("log", f"配置失败：{result.get('error', result)}")
        except Exception as exc:
    ...
    def _on_rpc_done(self, action, result):
        ...
        if isinstance(result, dict):
            self.log(result.get("message") or result.get("msg") or result.get("error") or str(result))
        else:
            self.log(str(result))
    ...
    def _render_state(self):
        with self._state_lock:
            ...
            movement_input_valid = bool(self.game_state.get("movementInputValid"))
        ...
        self.camera_label.configure(text="正常" if camera_valid and camera_initialized else "等待", text_color=COLOR_GREEN if camera_valid and camera_initialized else COLOR_ORANGE)
        input_ok = input_valid or movement_input_valid
        self.input_label.configure(text="正常" if input_ok else "等待", text_color=COLOR_GREEN if input_ok else COLOR_ORANGE)
        ...
        self._sync_switch_from_state()
        if last_error and last_error != self._last_error_shown:
            self._last_error_shown = last_error
            self.status_label.configure(text="已连接，但脚本有警告")
            self.log(f"脚本警告：{last_error}")
    ...
    def _shutdown_worker(self):
        try:
            with self._rpc_lock:
                if self.script:
                    try: self.script.exports_sync.cleanup()
                    except Exception: pass
