...
#
# 灵魂相机模式：
#   开启后相机位置独立维护，不再绑定人物位置。
#   W/S 按镜头 forward 飞行，抬头 W 上天，低头 W 入地。
#   A/D 水平左右平移。
#
...
JS_FILE = os.path.join(BASE_DIR, "free_camera_v1_3.js")
...
class FreeCameraApp(ctk.CTk):
    ...
    def setup_ui(self):
        ...
        ctk.CTkLabel(self, text="自由视角 v1.3", font=ctk.CTkFont(size=24, weight="bold")).grid(row=0, column=0, padx=18, pady=(16, 2), sticky="ew")
        ctk.CTkLabel(
            self,
            text="灵魂相机：不绑定人物。W/S 按镜头方向飞，抬头 W 上天，低头 W 入地；A/D 左右平移。",
        ...
        ctk.CTkLabel(config_frame, text="飞行速度", font=ctk.CTkFont(size=12)).grid(row=0, column=0, padx=12, pady=8, sticky="w")
        self.speed_var = ctk.DoubleVar(value=10.0)
        self.speed_slider = ctk.CTkSlider(config_frame, from_=1.0, to=50.0, number_of_steps=49, variable=self.speed_var, command=self.on_speed_changed)
        self.speed_slider.grid(row=0, column=1, padx=8, pady=8, sticky="ew")
        self.speed_label = ctk.CTkLabel(config_frame, text="10.0", width=50)
        self.speed_label.grid(row=0, column=2, padx=12, pady=8)

        self.lock_move_var = ctk.BooleanVar(value=True)
        self.lock_move_check = ctk.CTkCheckBox(config_frame, text="开启时尽量锁住玩家水平移动", variable=self.lock_move_var, command=self.on_lock_move_changed)
    ...
    def _render_state(self):
        with self._state_lock:
            ...
            input_mode = str(self.game_state.get("inputMode") or "none")
        ...
        self.input_label.configure(text=input_mode if input_ok else "等待", text_color=COLOR_GREEN if input_ok else COLOR_ORANGE)
