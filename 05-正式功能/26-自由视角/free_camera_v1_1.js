...
//   只使用固定 RVA、固定字段偏移、Hook；WASD 输入来自 Player.GetWalkVelocity。
// ============================================================
...
(function () {
    ...
    function getMovementInput(now) {
        if (!state.moveInput.valid ||
            now - state.moveInput.lastSampleMs > config.inputStaleMs) {
            return { x: 0.0, z: 0.0 };
        }
    ...
    function updateMovementInputFromWalkVelocity(returnBuffer, player, displayClass) {
        ...
        try {
            ...
            state.moveInput.x = inputX;
            state.moveInput.z = inputZ;
    ...
    function performEnable() {
        ...
        sendLog('info', '自由视角已开启', {
            move: 'W/A/S/D from Player.GetWalkVelocity',
            vertical: false,
    ...
    function installHooks() {
        ...
        try {
            ...
            attachHook('Player.GetWalkVelocity', RVA.Player_GetWalkVelocity, {
                ...
                onLeave: function () {
                    updateMovementInputFromWalkVelocity(
                        this.returnBuffer,
                        this.player,
                        this.displayClass
                    );
    ...
    sendLog('info', 'free_camera_v1_1.js loaded');
})();
