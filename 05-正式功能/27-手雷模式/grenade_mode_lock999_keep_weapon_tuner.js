...
function emit(level, message, data) {
    try { send({ feature_id: FEATURE_ID, version: VERSION, level: level || "INFO", time: nowIso(), message: message || "", data: data || {} }); } catch (e) {}
}
...
function recordError(where, e) {
    ...
    if (Runtime.stats.error_count >= Runtime.config.max_error_before_disable) {
        Runtime.enabled = false;
        warn("错误过多，功能已自动关闭", { error_count: Runtime.stats.error_count });
    }
}
...
function forceThrowReady(wpnThrowPtr, reason) {
    try {
        ...
        const scopeInfo = throwWeaponAllowedByScope(wpnThrowPtr, reason + '.throwReady');
...
function getThrowWeaponLocalInfo(wpnThrowPtr) {
    const info = { weapon: ptrStr(wpnThrowPtr), isReadable: false, isMyWeapon: null, allowedByInfiniteScope: false, scope: Runtime.config.infinite_grenade_scope || "local_only" };
    try {
...
function writeFloatIfNeeded(base, offset, value, label) {
    try {
        ...
        if (Math.abs(old - v) < 0.0001) return false;
        base.add(offset).writeFloat(v);
...
function writeShootSpeedIfNeeded(missile, reason) {
    ...
    try {
        ...
        if (Math.abs(old - v) < 0.0001) return false;
        missile.add(Offsets.Missile_shootSpeed).writeFloat(v);
        Runtime.stats.missile_speed_writes += 1;
        logOnce("shootSpeed:" + ptrStr(missile), "已修改手雷 missile 飞行速度", Runtime.config.log_interval_ms, { missile: ptrStr(missile), reason: reason, before: old, after: v });
        return true;
...
function tuneGrenadeRuntimeFields(grenade, reason, allowLog) {
    ...
    let writes = 0;
    if (allowDamage && writeFloatIfNeeded(grenade, Offsets.Grenade_expDamage, Runtime.config.damage_value, "expDamage")) writes++;
    if (allowRange && writeFloatIfNeeded(grenade, Offsets.Grenade_expRange, Runtime.config.range_value, "expRange")) writes++;
...
function writeCreateExplosionArgs(ctx, reason) {
    try {
        ...
        if (Runtime.config.enable_damage && item.allowDamage !== false) {
            ...
            if (isFinite(v) && Math.abs(damageBefore - v) > 0.0001) {
                pDamage.writeFloat(v);
                damageAfter = v;
                writes++;
            } else {
        ...
        if (Runtime.config.enable_range && item.allowRange !== false) {
            ...
            if (isFinite(v) && Math.abs(rangeBefore - v) > 0.0001) {
                pRange.writeFloat(v);
                rangeAfter = v;
                writes++;
            } else {
...
rpc.exports = { enable(){ return enableFeature(); }, disable(){ return disableFeature(); }, status(){ return getStatus(); }, cleanup(){ return cleanupFeature(); }, setConfig(config){ return setConfig(config); }, set_config(config){ return setConfig(config); }, debugDump(){ return getStatus(); }, debug_dump(){ return getStatus(); } };
