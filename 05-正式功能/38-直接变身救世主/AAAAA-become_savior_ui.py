...
class BecomeSaviorApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("直接变身救世主 - 独立测试")
        ...
        self._write_log_file(f"session_start feature=become_savior pid_file={self.log_path}")
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(
            self, text="直接变身救世主", font=ctk.CTkFont(size=24, weight="bold"),
        ).grid(row=0, column=0, padx=18, pady=(18, 4), sticky="ew")

        ctk.CTkLabel(
            self,
            text="多人生化下让本地/存活 Bot 变成救世主（需当前为佣兵）",
    ...
    def _open_log_file():
        ...
        path = os.path.join(LOG_DIR, f"become_savior_ui_{stamp}.log")
    ...
    def _render_status(self, status):
        ...
        self.local_label.configure(text=str(status.get("local_savior_applied", 0)))
        self.bot_label.configure(text=str(status.get("bot_savior_applied", 0)))
