...
SUCCESS_MARKERS = (
    "local became savior",
    "local_savior applied",
    "became savior + weapon",
    "role set, weapon failed",
    "already savior, weapon re-given",
)
...
def find_logs(all_logs: bool) -> list[Path]:
    if not LOG_DIR.exists():
        return []
    files = sorted(LOG_DIR.glob("become_savior_ui_*.log"))
    if all_logs:
        return files
    return files[-1:]
...
def evaluate(statuses: list[dict], raw: str) -> tuple[str, list[str], list[str]]:
    ...
    local_applied = int(last.get("local_savior_applied") or 0)
    bot_applied = int(last.get("bot_savior_applied") or 0)
    ...
    if last_error and "not in multiplayer-nano mode" in last_error:
        problems.append("当前不在多人生化 nano 模式，功能按设计拒绝执行")
    if last_error and "Mode_Nano4.instance3 is unavailable" in last_error:
        problems.append("Mode_Nano4 实例不可用，可能是模式未就绪或实例获取失败")
    ...
    if actions and any(s.get("pending") for s in statuses):
...
def report(path: Path, verdict: str, problems: list[str], notes: list[str]) -> int:
    ...
    if verdict == "INCOMPLETE":
        print("RESULT: INCOMPLETE（请游戏内实际点变身后再查）")
        return 0
    print("RESULT: FAIL")
    return 1
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.path:
        ...
        if not paths:
            ...
            print("  NOTE  请先运行 AAAAA-become_savior_ui.py 并完成游戏内测试")
            return 2
