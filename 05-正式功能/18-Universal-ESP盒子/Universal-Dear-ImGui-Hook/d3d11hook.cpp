...
namespace hooks_dx11 {
    ...
    static void RenderFrame(IDXGISwapChain* pSwapChain)
    {
        ...
        if (gInitialized)
        {
            ...
            static int espInitRetryCount = 0;
            static DWORD lastEspInitTime = 0;
            
            if (!espInitialized && espInitRetryCount < 10) {
                DWORD currentTime = GetTickCount();
                
                // Only retry every 2 seconds
                if (currentTime - lastEspInitTime >= 2000) {
                    lastEspInitTime = currentTime;
                    espInitRetryCount++;
                    
                    DebugLog("[d3d11hook] ESP init attempt %d/10\n", espInitRetryCount);
                    
                    HMODULE gameAsm = GetModuleHandleA("GameAssembly.dll");
                    DebugLog("[d3d11hook] GameAssembly.dll: 0x%p\n", gameAsm);
                    
                    if (gameAsm && esp::IL2CPPBridge::Initialize(gameAsm)) {
                        DebugLog("[d3d11hook] IL2CPPBridge initialized OK\n");
                        
                        if (esp::ESPRenderer::Initialize()) {
                            espInitialized = true;
                            DebugLog("[d3d11hook] ESP System initialized successfully!\n");
                        } else {
                            DebugLog("[d3d11hook] ERROR: ESPRenderer::Initialize() failed\n");
                        }
                    } else {
                        DebugLog("[d3d11hook] ERROR: IL2CPPBridge::Initialize() failed\n");
                    }
                }
            }
