"""Minimal CustomTkinter UI for wall_noclip."""
...
JS_FILE = os.path.join(BASE_DIR, "AAAAA-wall_noclip_min.js")
...
class WallNoclipApp(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(
            self,
            text="只处理本地玩家横向碰撞，保留游戏自身下落和跳跃逻辑",
            text_color="#8f98a8",
        ...
        self.applied_label = self._add_metric(metrics, 1, "运行状态", "未捕获")
        self.write_label = self._add_metric(metrics, 2, "穿墙写入", "0")
        ...
        self.detail_label.grid(row=5, column=0, padx=18, pady=(6, 4), sticky="ew")
    ...
    def _render_status(self, status):
        ...
        writes = status.get("noclip_writes", 0)
        last_error = status.get("last_error") or ""
        last_apply = status.get("lastApplyReason") or ""
        ...
        if applied:
            self.applied_label.configure(text="已捕获", text_color=GREEN)
        elif pending:
            self.applied_label.configure(text="等待进房", text_color=ORANGE)
        else:
            self.applied_label.configure(text="未捕获", text_color="#ccc")

        self.write_label.configure(text=str(writes), text_color=GREEN if writes else "#ccc")

        if last_error:
            self.detail_label.configure(text="错误：" + last_error, text_color=RED)
        elif enabled and applied:
            self.detail_label.configure(text="运行中：" + last_apply, text_color=GREEN)
        elif enabled:
            self.detail_label.configure(text="已开启，等待本地玩家移动后捕获 controller", text_color=ORANGE)
        else:
            self.detail_label.configure(text="关闭状态：游戏使用原始移动碰撞", text_color="#9aa3b2")
