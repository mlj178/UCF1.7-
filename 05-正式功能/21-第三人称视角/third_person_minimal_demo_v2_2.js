(function () {
    ...
    /*
     * UCF1.7 第三人称最小化 Demo v2.2（晚连接与自动启用修正版）
     *
     * 设计原则：
    ...
    function emitPitchTelemetry(forceReason) {
        ...
        sendLog('info', '[PitchTelemetry]', {
            ...
            cameraForwardY: Number(
                stats.lastCameraForwardY.toFixed(6)
            ),
            actualCameraPitchDeg: Number(
                stats.lastCameraPitch.toFixed(4)
            ),
            actualCameraPitchDeltaDeg: Number(
                stats.lastCameraPitchDelta.toFixed(4)
            ),
            actualCameraPitchChanged: cameraPitchActive,
            cameraPitchMin: telemetry.cameraPitchMin,
            cameraPitchMax: telemetry.cameraPitchMax,
            ...
            verdict: {
                ...
                cameraChanging: cameraPitchActive,
                yAxisWasOverwritten:
        ...
        try {
            ...
            attach(
                ...
                RVA.CameraManager_Update,
                {
                    ...
                    onLeave: function () {
                        if (!pendingEnable || enabled || enableInProgress) return;

                        stats.automaticEnableAttempts++;

                        // 当前 Hook 位于 Unity 主线程，适合执行模型和相机切换。
                        var result = enableDemo({
                            source: 'CameraManager.Update.auto'
                        });

                        if (result && result.enabled) {
                            stats.automaticEnableSuccesses++;
                            sendLog('info', '[Enable] 等待条件满足，已自动开启', {
                                attempts: stats.automaticEnableAttempts,
                                cameraManager:
                                    isNull(cameraManager)
                                        ? null
                                        : cameraManager.toString(),
            ...
            attach(
                ...
                RVA.Brain_PushStateToUnityCamera,
                {
                    ...
                    onLeave: function () {
                        ...
                        sampleActualCameraPitch(this.brain);
        ...
        getstatus: function () {
            return {
                ...
                design: {
                    ...
                    telemetry: '1Hz PitchTelemetry + actual camera pitch',
    ...
    sendLog('info', '第三人称最小化 Demo v2.2 脚本已加载', {
        base: base.toString(),
        arch: Process.arch,
        pointerSize: Process.pointerSize
    });
