...
SCRIPT_NAME = "third_person_basic_v3_3.js"
...
class ThirdPersonCombatUI(ctk.CTk):
    ...
    def __init__(self) -> None:
        ...
        self.title(f"第三人称基础稳定版 v{APP_VERSION}")
        ...
        self.emit_log("UI 已启动。v3.3 仅验证背后居中、固定距离、原生 FOV 和平滑俯仰。")
    ...
    def build_ui(self) -> None:
        ...
        combat_tab = self.tabview.add("高级（默认关闭）")
    ...
    def build_camera_tab(self, tab: Any) -> None:
        ...
        self.fov_slider.configure(state="disabled")
    ...
    def build_status_tab(self, tab: Any) -> None:
        ...
        fields = [
            ...
            ("游戏原生 FOV", "lastNativeFov"),
            ("镜头到枢轴距离", "lastCameraAnchorDistance"),
            ("FOV 强制写入", "finalCameraWrites"),
            ("纵向最大步长", "maxYAxisStep"),
        ]
    ...
    def load_config(self) -> dict[str, Any]:
        ...
        # 首次升级到 v3.3 时强制使用安全基础配置。
        if str(loaded.get("configVersion", "")) != "3.3":
            config.update(
                {
                    "shoulder": "center",
                    "distance": 3.0,
    ...
    def apply_status(self, status: Any) -> None:
        ...
        values = {
            ...
            "lastNativeFov": (
                f"{float(stats.get('lastNativeFov', 0.0)):.1f}°"
            ),
            "lastCameraAnchorDistance": (
                f"{float(stats.get('lastCameraAnchorDistance', 0.0)):.2f} m"
            ),
            "finalCameraWrites": stats.get(
                "finalCameraWrites",
                0,
            ),
            "maxYAxisStep": (
                f"{float(stats.get('maxYAxisStep', 0.0)):.4f}"
            ),
        }
