...
SCRIPT_NAME = "third_person_basic_v3_14_3_model_relative_movement.js"
...
CONFIG_PATH = APP_DIR / "third_person_basic_v3_14_3_config.json"
...
class MinimalTPSUI(ctk.CTk):
    ...
    def build_camera_tab(self, tab: Any) -> None:
        ...
        ctk.CTkLabel(
            frame,
            text=(
                "自由观察：相机可环绕人物，人物根朝向保持不动。\n"
                "移动：W/S/A/D 始终以人物模型实际面朝方向为基准。\n"
                "战斗：人物实际 forward 精确对齐准星世界目标。\n"
                "射击：相机中心目标点 → 枪口位置 → GetShootRay。"
            ),
            justify="left",
            wraplength=720,
            text_color=self.MUTED,
    ...
    def build_status_tab(self, tab: Any) -> None:
        ...
        fields = [
            ...
            ("水平角模式", "yawSource"),
            ("当前视角模式", "viewMode"),
            ("人物固定恢复", "freeOrbitRotationRestores"),
            ("战斗精确对齐", "combatRootAlignments"),
            ("移动基准", "movementBasis"),
        ...
        values = {
            ...
            "viewMode": (
                ...
                if (
                    status.get("runtime", {})
                    .get("viewMode", {})
                    .get("combatActive", False)
                )
                else "自由观察"
            ),
            "freeOrbitRotationRestores": stats.get(
                "freeOrbitRotationRestores",
                0,
            ),
            "combatRootAlignments": stats.get(
                "combatRootAlignments",
                0,
            ),
            "movementBasis": "人物模型方向",
