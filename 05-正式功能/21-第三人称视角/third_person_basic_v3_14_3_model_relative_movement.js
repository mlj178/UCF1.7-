(function () {
    ...
    /*
     * UCF1.7 最小可用第三人称射击版 v3.14.3（人物相对移动与准星精确对齐）
     *
     * 只保留：
     * 1. 第三人称 FreeLook 相机；
     * 2. 第三人称人物模型；
    ...
    var RVA = {
        ...
        Transform_get_rotation_Injected:       0x003F4380,
    ...
    var LIMITS = {
        ...
        /*
         * 真实开火后进入短暂战斗对齐状态。
         * 自动武器每发都会续期；停止射击后恢复自由观察。
         */
        combatHoldMs: 550,
        combatAlignmentEpsilon: 0.15,
        freeLookRestoreEpsilon: 0.0001,
    ...
    var runtime = {
        ...
        /*
         * explore：
         *   相机可以独立环绕，人物保持进入本帧前的水平朝向。
         *
         * combat：
         *   检测到本地真实开火后，人物水平朝向平滑对齐相机。
         */
        combat: {
            active: false,
            lastShotTime: 0,
    ...
    var stats = {
        ...
        modelDirectionUpdates: 0,
        modelDirectionFallbackReads: 0,

        combatEntries: 0,
        combatExits: 0,
        combatYawWrites: 0,
        combatRootAlignments: 0,
        combatAlignmentFailures: 0,
        freeLookYawRestores: 0,
        freeOrbitRotationCaptures: 0,
        freeOrbitRotationRestores: 0,
        freeOrbitRotationFailures: 0,
    ...
    /*
     * 自由观察时保存和恢复 characterContainer 的完整 Quaternion。
     * 只在 Player.UpdateCameraRotaion 的自由观察路径使用。
     */
    var freeOrbitRotationReadBuffer = Memory.alloc(16);
    var freeOrbitRotationWriteBuffer = Memory.alloc(16);
    ...
    var transformGetRotation = new NativeFunction(
        base.add(RVA.Transform_get_rotation_Injected),
        'void',
        ['pointer', 'pointer', 'pointer'],
        CALL_CONV
    );
    ...
    function updateBodyDirectionFromQuaternion(
        x,
        y,
        z,
        w
        ...
        /*
         * Unity Quaternion * Vector3.forward：
         *
         * forward.x = 2 * (x*z + w*y)
        ...
        if (!isFinite(length) ||
            length < 0.00001) {
            runtime.bodyDirection.valid = false;
            return false;
        }
        ...
        runtime.bodyDirection.valid = true;
        runtime.bodyDirection.forwardX =
            forwardX;
        runtime.bodyDirection.forwardZ =
            forwardZ;
        runtime.bodyDirection.yaw =
            normalizeAngle(
                Math.atan2(
                    forwardX,
                    forwardZ
        ...
        stats.modelDirectionUpdates++;
        return true;
    }

    function refreshBodyDirectionFromContainer(
        player
    ) {
        ...
        try {
            ...
            stats.modelDirectionUpdates++;
            stats.modelDirectionFallbackReads++;
            return true;
        } catch (_) {
            stats.errors++;
            return false;
    ...
    function resolveCombatAimDirection(
        characterContainer
    ) {
        var directionX = 0.0;
        var directionZ = 0.0;
         ...
         * 而不仅是相机自身的水平角。
         */
        if (runtime.aimSolution.valid &&
            readTransformPositionInto(
                characterContainer,
                tempCharacterPosition
            )) {
            directionX =
                runtime.aimSolution.targetX -
                tempCharacterPosition.x;

            directionZ =
                runtime.aimSolution.targetZ -
                tempCharacterPosition.z;
            ...
            if (isFinite(targetLength) &&
                targetLength >= 0.00001) {
                directionX /= targetLength;
                directionZ /= targetLength;
                valid = true;
            }
        }

        /*
         * 第一发目标尚未生成或目标位于人物正上/正下时，
         * 回退到最终相机的水平 forward。
         */
        if (!valid &&
            runtime.camera.valid) {
            directionX =
                runtime.camera.forward.x;

            directionZ =
                runtime.camera.forward.z;

            var cameraLength = Math.sqrt(
                directionX * directionX +
                directionZ * directionZ
            );

            if (isFinite(cameraLength) &&
                cameraLength >= 0.00001) {
                directionX /= cameraLength;
                directionZ /= cameraLength;
                valid = true;
            }
        }
    ...
    function alignCharacterRootToAim(
        characterContainer,
        rotationX,
        rotationY,
        rotationZ,
        ...
        if (isNull(characterContainer)) {
            stats.combatAlignmentFailures++;
            return false;
        }
        ...
        if (!isFinite(currentLength) ||
            currentLength < 0.00001) {
            stats.combatAlignmentFailures++;
            return false;
        }

        currentForwardX /= currentLength;
        currentForwardZ /= currentLength;

        var currentYaw = normalizeAngle(
            Math.atan2(
                currentForwardX,
                currentForwardZ
            ) * 180.0 / Math.PI
        ...
        var yawDelta = deltaAngle(
            currentYaw,
            target.yaw
        );

        if (Math.abs(yawDelta) <=
            LIMITS.combatAlignmentEpsilon) {
            runtime.bodyDirection.valid = true;
            runtime.bodyDirection.forwardX =
                target.x;
            runtime.bodyDirection.forwardZ =
        ...
        /*
         * 不把 targetYaw 当成模型 Quaternion 的绝对 Yaw。
         *
         * 而是把“人物当前实际 forward”旋转 yawDelta，
         * 因此会自动保留模型自身可能存在的固定轴偏移。
         *
         * 世界 Y 轴增量 Quaternion 左乘当前 Quaternion：
         * qNew = qDeltaYaw * qCurrent
         */
        var halfDelta =
            yawDelta *
            Math.PI / 360.0;
        ...
        var quaternionLength = Math.sqrt(
            newX * newX +
            newY * newY +
            newZ * newZ +
            newW * newW
        ...
        if (!isFinite(quaternionLength) ||
            quaternionLength < 0.00001) {
            stats.combatAlignmentFailures++;
            return false;
        }

        newX /= quaternionLength;
        newY /= quaternionLength;
        newZ /= quaternionLength;
        newW /= quaternionLength;

        freeOrbitRotationWriteBuffer.writeFloat(
            newX
        );
        freeOrbitRotationWriteBuffer.add(4).writeFloat(
            newY
        );
        freeOrbitRotationWriteBuffer.add(8).writeFloat(
            newZ
        );
        freeOrbitRotationWriteBuffer.add(12).writeFloat(
            newW
        );
        ...
        try {
            transformSetRotation(
                characterContainer,
                freeOrbitRotationWriteBuffer,
                ptr(0)
            );

            updateBodyDirectionFromQuaternion(
                newX,
                newY,
                newZ,
                newW
            ...
            stats.combatYawWrites++;
            stats.combatRootAlignments++;
            return true;
        } catch (_) {
            stats.errors++;
            stats.combatAlignmentFailures++;
    ...
    function rotateWalkVelocityToModel(
        returnBuffer,
        player,
        displayClass
    ) {
        ...
        try {
            var inputX =
                displayClass.readS32();

            var inputZ =
                displayClass.add(4).readS32();
            ...
            if (!runtime.bodyDirection.valid &&
                !refreshBodyDirectionFromContainer(
                    player
                )) {
                return false;
            }
            ...
            var originalX =
                returnBuffer.readFloat();

            var originalY =
                returnBuffer.add(4).readFloat();

            var originalZ =
                returnBuffer.add(8).readFloat();
            ...
            /*
             * W/S/A/D 基准改为人物模型实际 forward：
             *
             * forward = 人物面朝方向
             * right   = (forward.z, -forward.x)
             */
            var forwardX =
                runtime.bodyDirection.forwardX;
    ...
    function enterCombatMode(reason) {
        var now = Date.now();
        var combat = runtime.combat;

        if (!combat.active) {
            combat.active = true;
            combat.enteredAt = now;
            combat.reason = reason || 'shot';
        ...
        combat.lastShotTime = now;
    }

    function leaveCombatMode(reason) {
        var combat = runtime.combat;
        ...
        combat.active = false;
        combat.lastShotTime = 0;
        combat.enteredAt = 0;
        combat.reason = reason || 'idle';

        /*
         * 停火后保留人物最后战斗朝向。
         * 下次进入战斗时再从当时的人物朝向重新插值。
         */
    }

    function refreshCombatMode(now) {
        var combat = runtime.combat;

        if (combat.active &&
            now - combat.lastShotTime >
                LIMITS.combatHoldMs) {
            leaveCombatMode('shot_timeout');
        }

        return combat.active;
    }

    function handleCameraInputAndCombatState(
        player,
        previousYaw,
        havePreviousYaw
    ) {
        ...
        try {
            ...
            var combatActive =
                refreshCombatMode(now);

            if (!combatActive &&
                havePreviousYaw &&
                isFinite(previousYaw) &&
                Math.abs(
                    deltaAngle(
                        previousYaw,
                        rawYaw
                    ...
                    LIMITS.freeLookRestoreEpsilon) {
                /*
                 * 自由观察只恢复控制字段的水平角。
                 * 人物根 Quaternion 在 Hook onLeave 中单独恢复。
                 */
                player.add(
                    OFF.Player_cameraRotation
                ).writeFloat(previousYaw);

                runtime.cameraInput.rawYaw =
                    previousYaw;

                stats.freeLookYawRestores++;
            }
        } catch (_) {
            stats.errors++;
        }
    ...
    function emitBasicStatus() {
        ...
        sendLog('info', '[TPSStatus]', {
            ...
            freeOrbitRotationRestores:
                stats.freeOrbitRotationRestores,

            freeOrbitRotationFailures:
                stats.freeOrbitRotationFailures,

            combatYawWrites:
                stats.combatYawWrites,

            combatRootAlignments:
                stats.combatRootAlignments,

            combatAlignmentFailures:
                stats.combatAlignmentFailures,
    ...
    function resetRuntime() {
        ...
        runtime.bodyDirection.valid = false;
        runtime.bodyDirection.forwardX = 0.0;
        runtime.bodyDirection.forwardZ = 1.0;
        runtime.bodyDirection.yaw = 0.0;

        runtime.combat.active = false;
        runtime.combat.lastShotTime = 0;
        runtime.combat.enteredAt = 0;
        runtime.combat.reason = 'idle';
    ...
    function installHooks() {
        ...
        try {
            ...
            /*
             * 人物模型相对 WASD：
             * W/S/A/D 使用 characterContainer 的实际世界 forward，
             * 与自由环绕相机方向完全解耦。
             */
            attach(
             ...
             * 自由观察与战斗对齐：
             *
             * v3.14.1 只恢复 cameraRotation.x，但原函数内部已经可能把
             * 水平朝向应用到 characterContainer，所以模型仍会转动。
             *
             * v3.14.2 在自由观察状态额外执行：
             *   1. onEnter 保存 characterContainer 的完整 Quaternion；
             *   2. 原函数正常更新 Cinemachine 相机输入；
             *   3. onLeave 恢复 cameraRotation.x；
             *   4. onLeave 恢复 characterContainer Quaternion。
             *
             * 战斗状态根据准星世界目标旋转人物根节点。
             */
            attach(
                ...
                RVA.Player_UpdateCameraRotaion,
                {
                    onEnter: function (args) {
                        ...
                        this.havePreviousYaw = false;
                        this.previousYaw = 0.0;

                        this.restoreRootRotation = false;
                        this.characterContainer = ptr(0);
                         ...
                         *
                         * free_orbit：原样恢复；
                         * combat_align：以此 Quaternion 为起点，
                         * 精确旋转到准星世界目标方向。
                         */
                        refreshCombatMode(
                            Date.now()
                        ...
                        try {
                            ...
                            transformGetRotation(
                                container,
                                freeOrbitRotationReadBuffer,
                                ptr(0)
                            );

                            var x =
                                freeOrbitRotationReadBuffer.readFloat();

                            var y =
                                freeOrbitRotationReadBuffer.add(4).readFloat();

                            var z =
                                freeOrbitRotationReadBuffer.add(8).readFloat();

                            var w =
                                freeOrbitRotationReadBuffer.add(12).readFloat();
                            ...
                            stats.freeOrbitRotationCaptures++;
                        } catch (_) {
                            stats.freeOrbitRotationFailures++;
                        }
                    },
                    ...
                    onLeave: function () {
                        handleCameraInputAndCombatState(
                            this.player,
                            this.previousYaw,
                            this.havePreviousYaw
                        );

                        if (!this.restoreRootRotation ||
                            isNull(
                                this.characterContainer
                            )) {
                            return;
                        ...
                        if (runtime.combat.active) {
                            /*
                             * 战斗状态：
                             * 直接用人物当前实际 forward 到准星世界目标
                             * 的角差旋转根节点，不再写 cameraRotation.x。
                             */
                            alignCharacterRootToAim(
                                this.characterContainer,
                                this.rotationX,
                                this.rotationY,
                                this.rotationZ,
        ...
        getstatus: function () {
            return {
                ...
                runtime: {
                    ...
                    viewMode: {
                        ...
                        combatActive:
                            runtime.combat.active,
                        lastShotTime:
                            runtime.combat.lastShotTime,
                        reason:
                            runtime.combat.reason
                    },
                    refreshQueue: {
                        pending:
                            refreshQueue.pending,
                ...
                design: {
                    ...
                    viewModes:
                        'free orbit while idle; character aligns only during real local fire',
                    freeOrbitBodyLock:
                        'restore Player.characterContainer Quaternion after camera input',
                    movementBasis:
                        'characterContainer actual world forward',
                    combatAlignment:
                        'rotate current root Quaternion by actual-forward to crosshair-target yaw delta',
                    combatExit:
                        'retain last combat body yaw and return to free orbit after timeout',
                    lifecycle:
