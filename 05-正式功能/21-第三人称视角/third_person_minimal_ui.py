...
SCRIPT_NAME = "third_person_minimal_demo.js"
...
class MinimalThirdPersonUI(ctk.CTk):
    ...
    def build_ui(self) -> None:
        ...
        ctk.CTkLabel(
            title_box,
            text="原生 FreeLook + 原生 Collider；不修改 WASD、人物 Yaw 和射击",
    ...
    def load_config(self) -> dict[str, Any]:
        ...
        config["pitchSpeed"] = self.clamp(
            config.get("pitchSpeed"),
            0.1,
            5.0,
            2.0,
        )
    ...
    def apply_config_to_controls(self) -> None:
        ...
        try:
            ...
            self.pitch_slider.set(self.config_data["pitchSpeed"])
    ...
    def refresh_value_labels(self) -> None:
        ...
        self.pitch_value.configure(
            text=f"{self.config_data['pitchSpeed']:.1f}"
        )
    ...
    def on_pitch_changed(self, value: float) -> None:
        if self.syncing_controls:
            return
        self.config_data["pitchSpeed"] = round(float(value), 2)
        self.refresh_value_labels()
        self.config_changed()
    ...
    def apply_status(self, status: Any) -> None:
        ...
        values = {
            ...
            "camera": (
                "已获取"
                if status.get("haveCameraManager")
                else "未获取"
            ),
            "player": (
                "已获取"
                if status.get("haveLocalPlayer")
                else "未获取"
            ),
            "cameraUpdates": stats.get("cameraUpdates", 0),
