(function () {
     ...
     * 3. 只修改 FreeLook 轨道、Composer 构图、YAxis 输入和 FOV。
    ...
    function captureYAxis(freeLook) {
        ...
        if (!ensureMouseYString()) return false;
    ...
    function applyYAxis(freeLook) {
        if (!enabled) return;
        if (!captureYAxis(freeLook)) return;

        guarded('应用 YAxis', function () {
            var axis = freeLook.add(OFF.CFL_m_YAxis);

            axis.add(OFF.AX_InputAxisName).writePointer(mouseYString);
            axis.add(OFF.AX_InputProvider).writePointer(ptr(0));
            axis.add(OFF.AX_InputAxisValue).writeFloat(0.0);
            axis.add(OFF.AX_MaxSpeed).writeFloat(config.pitchSpeed);
            axis.add(OFF.AX_InvertInput).writeU8(config.invertY ? 1 : 0);
    ...
    function validateConfig(input) {
        ...
        error = numberField('pitchSpeed', 0.1, 5.0);
        if (error) return { ok: false, error: error };
                ...
                RVA.CameraManager_Update,
                {
                    ...
                    onLeave: function () {
                        ...
                        if (!isNull(freeLook) && isReadable(freeLook)) {
                            // GameManager.Update 会重写 MaxSpeed；
                            // 因此这里只做轻量内存维护，不调用 Unity API。
                            applyYAxis(freeLook);
                        }
                    }
                }
    ...
    sendLog('info', '第三人称最小化 Demo 脚本已加载', {
        base: base.toString(),
        arch: Process.arch,
        pointerSize: Process.pointerSize
    });
