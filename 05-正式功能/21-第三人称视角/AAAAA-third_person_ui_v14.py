...
class ThirdPersonApp(ctk.CTk):
    ...
    def _build_diag_tab(self) -> None:
        ...
        self.diag_text.insert(
            "end",
            "进入对局后可执行模型诊断。\n"
            "安全诊断只在 Cinemachine Brain 主线程读取 Unity 对象，可测试腰射、ADS、切枪和纵向输入。\n",
        )
    ...
    def _on_script_message(self, message: dict[str, Any], data: Any) -> None:
        ...
        if message_type == "send":
            ...
            if isinstance(payload, dict) and payload.get("type") == "safe_diag_result":
                result = payload.get("result", {})
                self._schedule_ui(
                    lambda result=result: self._show_diag_result(
                        "安全主线程快照",
                        result,
                    )
                )
    ...
    def _debug_model_requested(self) -> None:
        self._run_rpc_async(
            "debugmodel",
            on_success=lambda result: self._show_diag_result("安全模型/相机快照", result),
            description="安全模型快照",
        )
    ...
    def _start_aim_diag_requested(self) -> None:
        ...
        def completed(result: Any) -> None:
            ...
            self._show_diag_result("开始安全诊断", result)
            if isinstance(result, dict) and result.get("ok"):
                self._emit_log(f"安全主线程诊断已启动，时长 {seconds} 秒。")
            else:
