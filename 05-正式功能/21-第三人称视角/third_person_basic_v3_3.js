(function () {
    ...
    var pendingShoulderApply = false;
    ...
    var config = {
        ...
        // 兼容旧配置；v3.3 不再强制写输出 Camera 的 FOV。
        fieldOfView: 70.0,
        pitchScale: 1.0,
        invertY: false,

        // 高级战斗功能默认关闭，先验证基础第三人称。
        dynamicShoulder: false,
        shootingRay: false,
        physicsAimRaycast: false,
    ...
    var PERF = {
        ...
        fovCheckIntervalMs: 500,
        combatLogIntervalMs: 2000,
        visualLogIntervalMs: 2000,
        colliderLogIntervalMs: 2000,

        shoulderHotkeyCooldownMs: 700,
        pitchSmoothingTimeMs: 100,
        pitchDeadzone: 0.0006,
        pitchMaxStepPerUpdate: 0.055
    };
    ...
    var stats = {
        ...
        shoulderHotkeyEdges: 0,
        shoulderHotkeySuppressed: 0,
        shoulderLightweightApplies: 0,
        shoulderFullReapplyAvoided: 0,

        pitchSmoothingWrites: 0,
        pitchTargetJumps: 0,
        lastYAxisTarget: 0.5,
        lastYAxisSmoothed: 0.5,
        lastYAxisStep: 0.0,
        maxYAxisStep: 0.0,
        ...
        cameraAnchorDistanceSamples: 0,
        lastCameraAnchorDistance: 0.0,
        cameraAnchorDistanceMin: 999.0,
        cameraAnchorDistanceMax: 0.0,
    ...
    var combat = {
        ...
        inputState: {
            shoulderKeyHeld:false,
            lastShoulderSwapTime:0
        },
        cameraStability: {
            yAxisLockedFreeLook:null,
            yAxisLockedRevision:-1,
    ...
    var inputGetKey = new NativeFunction(
        base.add(RVA.Input_GetKey), 'bool', ['int','pointer'], CALL_CONV
    );
    ...
    function updateInputFeaturesOnMainThread() {
        ...
        var now = Date.now();
        if (now - combat.timing.lastInput <
            PERF.inputIntervalMs) {
            return;
        }
        combat.timing.lastInput = now;

        if (config.adsFirstPerson) {
            var adsPressed = guarded(
                '读取鼠标右键',
                function () {
            ...
            if (adsPressed && !combat.adsActive) {
                enterAdsOnMainThread();
            }
            if (!adsPressed && combat.adsActive) {
                exitAdsOnMainThread();
            }
        ...
        var inputState = combat.inputState;

        if (!config.dynamicShoulder || combat.adsActive) {
            inputState.shoulderKeyHeld = false;
            return;
        }
        ...
        var keyHeld = guarded(
            '读取 V 键状态',
            function () {
                return inputGetKey(118, ptr(0));
            },
            false
        );

        var risingEdge =
            keyHeld && !inputState.shoulderKeyHeld;
        inputState.shoulderKeyHeld = keyHeld;

        if (!risingEdge) return;

        stats.shoulderHotkeyEdges++;

        var elapsed =
            now - inputState.lastShoulderSwapTime;

        if (elapsed < PERF.shoulderHotkeyCooldownMs) {
            stats.shoulderHotkeySuppressed++;
            sendLogRateLimited(
                'shoulder_hotkey_suppressed',
                2000,
                'warning',
                '[CameraStability] 已拦截重复换肩输入',
                {
                    elapsedMs: elapsed,
                    cooldownMs:
                        PERF.shoulderHotkeyCooldownMs,
                    suppressed:
                        stats.shoulderHotkeySuppressed
                }
            );
        ...
        inputState.lastShoulderSwapTime = now;
        config.shoulder =
            config.shoulder === 'left'
                ? 'right'
                : 'left';

        pendingShoulderApply = true;
        stats.shoulderSwapCount++;

        sendLog('info', '[Shoulder] V 键换肩请求', {
            shoulder: config.shoulder,
            count: stats.shoulderSwapCount,
            inputMode:
                'rising_edge_with_700ms_cooldown'
        });

        send({
            type:'shoulder_changed',
            shoulder:config.shoulder
        });
    }
    ...
    function sampleCameraFoundationMetrics() {
        if (!enabled ||
            !combat.finalCamera.valid ||
            combat.finalCamera.position === null) {
            return null;
        ...
        var player = resolveLocalPlayer();
        if (isNull(player) || !isReadable(player)) return null;

        return guarded(
            '采样镜头到上背部枢轴距离',
            function () {
                var container = player.add(
                ...
                var anchor = safeTransformPosition(container);
                if (!anchor.valid) return null;

                anchor.y += config.pivotHeight;

                var distance = v3Distance(
                    combat.finalCamera.position,
                    anchor
                );
                ...
                stats.lastCameraAnchorDistance = distance;
                stats.cameraAnchorDistanceSamples++;
                stats.cameraAnchorDistanceMin = Math.min(
                    stats.cameraAnchorDistanceMin,
                    distance
                );
                stats.cameraAnchorDistanceMax = Math.max(
                    stats.cameraAnchorDistanceMax,
                    distance
                );

                return {
                    distance: distance,
                    min: stats.cameraAnchorDistanceMin,
                    max: stats.cameraAnchorDistanceMax,
                    range:
                        stats.cameraAnchorDistanceMax -
                        stats.cameraAnchorDistanceMin
                };
    ...
    function updateCombatRuntimeOnMainThread() {
        ...
        if (combat.pendingLifecycleRefresh) {
            ...
            if (!isNull(player)) {
                ...
                // 切枪、出生和复活只刷新模型和武器缓存。
                // 不重复写 Follow、LookAt、Orbit 或重新激活 FreeLook。
        ...
        if (now - combat.lastCombatLog >= PERF.combatLogIntervalMs) {
            ...
            var foundationMetrics =
                sampleCameraFoundationMetrics();

            sendLog('info', '[CameraFoundation]', {
                mode: 'basic_third_person',
                anchor:
                    'characterContainer + upperBackOffset',
                centeredBehindBack:
                ...
                configuredDistance:
                    config.distance,
                measuredCameraDistance:
                    foundationMetrics !== null
                        ? foundationMetrics.distance
                        : null,
                measuredDistanceRange:
                    foundationMetrics !== null
                        ? foundationMetrics.range
                        : null,
                orbitRadii: [
                    config.distance,
                    config.distance,
                ...
                nativeFov:
                    stats.lastNativeFov > 0
                        ? stats.lastNativeFov
                        : null,
                nativeFovRange:
                    stats.nativeFovSamples > 0
                        ? [
                            stats.nativeFovMin,
                            stats.nativeFovMax
                        ]
                ...
                fovPolicy:
                    'native_observe_only',
                pitchTarget:
                    stats.lastYAxisTarget,
                pitchSmoothed:
                    stats.lastYAxisSmoothed,
                pitchMaxStep:
                    stats.maxYAxisStep
            });
            ...
            sendLog('info', '[CombatTelemetry]', {
                ...
                performance:{
                    ...
                    suppressedLogs:
                        stats.logMessagesSuppressed,
                    nativeFovSamples:
                        stats.nativeFovSamples,
                    nativeFovChanges:
                        stats.nativeFovChanges,
                    outputFovWrites:
                        stats.finalCameraWrites,
                    cameraAnchorDistance:
                        stats.lastCameraAnchorDistance
                }
            });
        }
    }
        ...
        sendLog('info', '[PitchTelemetry]', {
            ...
            cameraStability: {
                ...
                maxYAxisStep:
                    stats.maxYAxisStep,
                pitchTargetJumps:
                    stats.pitchTargetJumps,
                smoothingTimeMs:
                    PERF.pitchSmoothingTimeMs,
                shoulder:
                    config.shoulder,
                dynamicShoulder:
                    config.dynamicShoulder,
                shoulderKeyHeld:
                    combat.inputState.shoulderKeyHeld,
                shoulderSwapCount:
                    stats.shoulderSwapCount,
                hotkeySuppressed:
                    stats.shoulderHotkeySuppressed,
                lightweightApplies:
                    stats.shoulderLightweightApplies,
                fullReapplyAvoided:
                    stats.shoulderFullReapplyAvoided
            },
            config: {
    ...
    function lockYAxisForDirectDrive(freeLook) {
        ...
        var stability = combat.cameraStability;
        var alreadyLocked =
            !isNull(stability.yAxisLockedFreeLook) &&
            stability.yAxisLockedFreeLook.equals(freeLook) &&
            stability.yAxisLockedRevision === configRevision;
        ...
        return guarded('锁定 YAxis 为直接驱动模式', function () {
            ...
            stability.yAxisLockedFreeLook = freeLook;
            stability.yAxisLockedRevision = configRevision;
            return true;
        }, false);
    }
    ...
    function driveVerticalPitch(freeLook) {
        ...
        var result = guarded(
            '平滑同步 Player pitch 到 FreeLook YAxis',
            function () {
                ...
                target = Math.max(
                    0.02,
                    Math.min(0.98, clamp01(target))
                );

                var axis = freeLook.add(OFF.CFL_m_YAxis);
                var beforeWrite = axis.add(
                    OFF.AX_Value
                ).readFloat();
                ...
                if (!stability.pitchInitialized ||
                    !isFinite(stability.smoothedYAxis)) {
                    stability.smoothedYAxis =
                        isFinite(beforeWrite)
                            ? beforeWrite
                            : target;
                    stability.pitchInitialized = true;
                    stability.lastPitchUpdateTime = now;
                }
                ...
                var dtMs = Math.max(
                    ...
                    Math.min(
                        100,
                        now - stability.lastPitchUpdateTime
                    )
                );
                stability.lastPitchUpdateTime = now;

                var alpha = 1.0 - Math.exp(
                    -dtMs / PERF.pitchSmoothingTimeMs
                );

                var difference =
                    target - stability.smoothedYAxis;
                var step = difference * alpha;

                step = Math.max(
                    -PERF.pitchMaxStepPerUpdate,
                    Math.min(
                        PERF.pitchMaxStepPerUpdate,
                        step
                    )
                ...
                if (Math.abs(difference) <= PERF.pitchDeadzone) {
                    stability.smoothedYAxis = target;
                    step = 0.0;
                } else {
                    stability.smoothedYAxis += step;
                }

                stability.smoothedYAxis = Math.max(
                    0.02,
                    Math.min(0.98, stability.smoothedYAxis)
                );
                stability.targetYAxis = target;

                axis.add(OFF.AX_Value).writeFloat(
                    stability.smoothedYAxis
                );
                axis.add(OFF.AX_InputAxisValue).writeFloat(0.0);
                axis.add(OFF.AX_MaxSpeed).writeFloat(0.0);

                var afterWrite = axis.add(
                    OFF.AX_Value
                ).readFloat();

                stats.lastYAxisBeforeWrite = beforeWrite;
                stats.lastYAxisWritten = afterWrite;
                stats.lastYAxisTarget = target;
                stats.lastYAxisSmoothed = afterWrite;
                stats.lastYAxisStep = step;
                stats.maxYAxisStep = Math.max(
                    stats.maxYAxisStep,
                    Math.abs(step)
                );
                stats.pitchSmoothingWrites++;
    ...
    function applyShoulderOnlyOnMainThread(reason) {
        if (!enabled ||
            combat.adsActive ||
            !saved.valid ||
            saved.composers.length < 1) {
            return false;
        }
        ...
        sendLog('info', '[CameraStability] 轻量换肩完成', {
        ...
        return guarded('应用最小相机配置', function () {
            ...
            if (!isNull(orbitArray) && isReadable(orbitArray)) {
                ...
                if (count >= 3) {
                    ...
                    // 三层 Rig 使用完全相同的半径。
                    // 俯仰时只改变高度和角度，不再改变镜头距离。
                    orbitArray.add(0x24).writeFloat(config.distance);
                }
            }
            ...
            sendLog('info', '[MainThread] 相机配置完成', {
                freeLook: freeLook.toString(),
                foundationMode: true,
                anchor: 'characterContainer + upperBackOffset',
                centeredBehindBack:
                    config.shoulder === 'center',
                shoulder: config.shoulder,
                screenX: targetScreenX,
    ...
    function enforceFov(brain) {
        ...
        guarded('采样游戏原生 FOV', function () {
            ...
            var previous = stats.lastNativeFov;
            stats.lastNativeFov = current;
            stats.nativeFovSamples++;
            stats.nativeFovMin = Math.min(
                stats.nativeFovMin,
                current
            );
            stats.nativeFovMax = Math.max(
                stats.nativeFovMax,
                current
            );

            if (previous > 0.0 &&
                Math.abs(current - previous) >= 0.5) {
                stats.nativeFovChanges++;
                sendLogRateLimited(
                    'native_fov_change',
                    2000,
                    'info',
                    '[CameraFoundation] 游戏原生 FOV 变化',
                    {
                        previous: previous,
                        current: current,
                        changeCount:
                            stats.nativeFovChanges,
                        policy:
                            'observe_only_no_output_write'
                    }
                );
            }

            // 不调用 Camera.set_fieldOfView。
            // 避免脚本与 Cinemachine 交替覆盖造成呼吸式缩放。
        }, null);
    }
    ...
    function resetCombatRunState() {
        var fields = [
            ...
            'shoulderSwapCount',
            'shoulderHotkeyEdges',
            'shoulderHotkeySuppressed',
            'shoulderLightweightApplies',
            'shoulderFullReapplyAvoided',
            'pitchSmoothingWrites',
            'pitchTargetJumps',
            'colliderCallbacks',
        ...
        stats.nativeFovSamples = 0;
        stats.nativeFovChanges = 0;
        stats.lastNativeFov = 0.0;
        stats.nativeFovMin = 999.0;
        stats.nativeFovMax = 0.0;

        stats.cameraAnchorDistanceSamples = 0;
        stats.lastCameraAnchorDistance = 0.0;
        stats.cameraAnchorDistanceMin = 999.0;
        stats.cameraAnchorDistanceMax = 0.0;
        ...
        combat.inputState.shoulderKeyHeld = false;
        combat.inputState.lastShoulderSwapTime = 0;

        combat.cameraStability.yAxisLockedFreeLook = null;
        combat.cameraStability.yAxisLockedRevision = -1;
        combat.cameraStability.pitchInitialized = false;
        combat.cameraStability.targetYAxis = 0.5;
        combat.cameraStability.smoothedYAxis = 0.5;
        combat.cameraStability.lastPitchUpdateTime = 0;

        stats.lastYAxisTarget = 0.5;
        stats.lastYAxisSmoothed = 0.5;
        stats.lastYAxisStep = 0.0;
        stats.maxYAxisStep = 0.0;
    ...
    function processMainThreadCommands() {
        ...
        if (pendingShoulderApply && enabled) {
            pendingShoulderApply = false;
            lastMainThreadCommand = 'shoulder_only';
            applyShoulderOnlyOnMainThread(
                'command_queue'
            );
            return;
        }
    ...
    function installHooks() {
        ...
        try {
            ...
            installed = true;
            sendLog('info', '第三人称基础版 Hook 安装完成');
            return { ok: true, installed: true };
        } catch (error) {
            stats.errors++;
    ...
    rpc.exports = {
        ...
        setconfig: function (input) {
            ...
            return {
                ...
                queued:
                    enabled &&
                    (shoulderChanged || cameraChanged),
                config: config,
                shoulderOnly: shoulderChanged,
                cameraReapply: cameraChanged,
                message:
                    enabled && cameraChanged
                        ? '相机参数已保存，将在主线程重新应用'
                        : enabled && shoulderChanged
                            ? '肩位已保存，将执行轻量换肩'
                            : '参数已保存，无需重建相机'
            };
        },
        ...
        getstatus: function () {
            return {
                ...
                design: {
                    ...
                    cameraFoundation:
                        'centered upper-back pivot + constant orbit radius',
                    fovPolicy:
                        'observe native FOV; never force output Camera',
                    verticalPitch:
                        'Player.cameraRotation.y -> smoothed FreeLook.YAxis.Value',
    ...
    sendLog('info', '第三人称基础稳定版 v3.3 脚本已加载', {
        base: base.toString(),
        arch: Process.arch,
        pointerSize: Process.pointerSize
    });
