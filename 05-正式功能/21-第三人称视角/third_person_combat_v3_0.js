(function () {
    ...
    var combat = {
        ...
        physics: { consecutiveFailures:0, disabledByError:false },
        ...
        lastCollisionLog: 0,
        lastFadeLog: 0,
        collider: { pointer:null, vcam:null, original:null, displaced:false, distance:0, queryFailures:0, telemetryDisabled:false },
    ...
    function clearBuffer(pointer, size) {
        Memory.writeByteArray(pointer, new Uint8Array(size));
    }
    ...
    function physicsRaycastMainThread(origin, direction, maxDistance) {
        ...
        try {
            ...
            clearBuffer(physicsSceneBuffer, 4);
            clearBuffer(physicsRayBuffer, 24);
            ...
            if (combat.physics.consecutiveFailures <= 3) {
                sendLog('error', '[PhysicsAim] 射线调用失败', {
                    error: String(error),
                    consecutiveFailures:
                        combat.physics.consecutiveFailures
                });
            ...
            if (combat.physics.consecutiveFailures >= 3) {
                ...
                sendLog('warning', '[PhysicsAim] 已自动禁用并回退到远点目标', {
    ...
    function safeTransformPosition(transform) {
        ...
        return guarded('读取 Transform.position', function () {
            var ret = Memory.alloc(12);
            transformGetPosition(transform, ret, ptr(0));
            return readVector3(ret);
    ...
    function safeTransformForward(transform) {
        ...
        return guarded('读取 Transform.forward', function () {
            var ret = Memory.alloc(12);
            // 该 getter 在目标 x86 ABI 中使用 hidden return buffer 在第一个参数。
            transformGetForward(ret, transform, ptr(0));
            var value = readVector3(ret);
            return value.valid ? v3Normalize(value) : value;
        }, { x:0, y:0, z:1, valid:false });
    }

    function sampleFinalCameraOnMainThread(brain) {
        ...
        try {
            ...
            combat.finalCamera.timestamp = Date.now();
            combat.finalCamera.consecutiveFailures = 0;
            ...
            if (targetHit.hit) {
                ...
                combat.aimTarget.point = v3Add(
                    position,
                    v3Scale(forward, config.maxAimDistance)
                );
                stats.aimRaycastMisses++;
            }
            combat.aimTarget.timestamp = Date.now();
            ...
            if (combat.finalCamera.consecutiveFailures <= 3) {
                sendLog('error', '[FinalCamera] 采样失败', {
                    error: String(error),
                    consecutiveFailures:
                        combat.finalCamera.consecutiveFailures
                });
    ...
    function updateMuzzleOnMainThread() {
    ...
    function updateShootSolutionOnMainThread() {
    ...
    function isLocalCharacter(candidate) {
        var player = resolveLocalPlayer();
        if (isNull(player) || isNull(candidate)) return false;
        try {
            var character = player.add(OFF.Player_currentCharacter).readPointer();
            return !isNull(character) && character.equals(candidate);
        } catch (_) {
            return false;
        }
    }
    ...
    function applyColliderSettings(collider) {
        if (!config.collisionEnhance || isNull(collider) || !isReadable(collider)) return;
        if (isNull(combat.collider.pointer) || !combat.collider.pointer.equals(collider)) {
            combat.collider.pointer = collider;
            combat.collider.original = {
                minimumDistance:collider.add(OFF.Collider_MinimumDistanceFromTarget).readFloat(),
                avoidObstacles:collider.add(OFF.Collider_AvoidObstacles).readU8() !== 0,
                cameraRadius:collider.add(OFF.Collider_CameraRadius).readFloat(),
                damping:collider.add(OFF.Collider_Damping).readFloat(),
                dampingOccluded:collider.add(OFF.Collider_DampingWhenOccluded).readFloat()
            };
            sendLog('info', '[Collider] 原始配置已保存', combat.collider.original);
        }
        collider.add(OFF.Collider_AvoidObstacles).writeU8(1);
        collider.add(OFF.Collider_CameraRadius).writeFloat(config.collisionRadius);
        collider.add(OFF.Collider_Damping).writeFloat(config.collisionDamping);
        collider.add(OFF.Collider_DampingWhenOccluded).writeFloat(config.collisionOccludedDamping);
    }
    ...
    function updateCombatRuntimeOnMainThread() {
        ...
        if (!config.occlusionFade && combat.renderers.captured) {
            restoreCharacterVisibility();
            sendLog('info', '[OcclusionFade] 已按配置关闭并恢复人物材质');
        }
        ...
        updateMuzzleOnMainThread();
        updateShootSolutionOnMainThread();
        ...
        if (combat.pendingLifecycleRefresh) {
            ...
            if (!isNull(player)) {
                ...
                sendLog('info', '[WeaponLifecycle] 生命周期重新绑定完成', {
        ...
        var now = Date.now();
        if (now - combat.lastCombatLog >= 1000) {
            ...
            attach(
                ...
                RVA.Brain_PushStateToUnityCamera,
                {
                    ...
                    onLeave: function () {
                        ...
                        sampleFinalCameraOnMainThread(this.brain);
                        updateOcclusionFadeOnMainThread();
                ...
                RVA.CinemachineCollider_PostPipelineStageCallback,
                {
                    ...
                    onLeave: function () {
                        if (!this.active ||
                            ...
                            isNull(this.vcam) ||
                            !isReadable(this.collider) ||
                            !isReadable(this.vcam)) {
                            return;
                        }

                        if (combat.collider.telemetryDisabled) return;

                        var colliderReadOk = guarded('读取 CinemachineCollider 位移', function () {
                            var displaced = colliderCameraWasDisplaced(
                                this.collider,
                                this.vcam,
                                ptr(0)
                            );
                            var distance = colliderGetDisplacementDistance(
                                this.collider,
                                this.vcam,
                                ptr(0)
                            );
                            ...
                            if (displaced) {
                                ...
                                if (distance > stats.colliderMaxDisplacement) {
                                    stats.colliderMaxDisplacement = distance;
                                }
                            }

                            var now = Date.now();
                            if (displaced &&
                                now - combat.lastCollisionLog >= 500) {
                                combat.lastCollisionLog = now;
                                sendLog('info', '[ColliderTelemetry]', {
                                    displaced: displaced,
                        ...
                        if (!colliderReadOk) {
                            ...
                            if (combat.collider.queryFailures >= 3) {
                                ...
                                sendLog('warning', '[Collider] 位移查询已自动禁用', {
                                    reason: 'three_consecutive_failures'
                                });
                            }
                        }
            ...
            attach(
                ...
                RVA.CharacterModel_LateUpdate,
                {
                    ...
                    onLeave: function () {
                        ...
                        var now = Date.now();
                        if (now - combat.lastIKLog < 1000) return;
    ...
    rpc.exports = {
        ...
        setconfig: function (input) {
            ...
            if (
                input.physicsAimRaycast === true &&
                combat.physics.disabledByError
            ) {
                combat.physics.disabledByError = false;
                combat.physics.consecutiveFailures = 0;
                sendLog('info', '[PhysicsAim] 用户重新启用物理射线熔断器');
            }
    ...
    sendLog('info', '第三人称战斗集成验证版 v3.0 脚本已加载', {
        base: base.toString(),
        arch: Process.arch,
        pointerSize: Process.pointerSize
    });
